...
{
public:
    QXmlSerializer(ref const(QXmlQuery) query,
                   QIODevice *outputDevice);

    /+virtual+/ void namespaceBinding(ref const(QXmlName) nb);
...
private:
     ...
     */
    void writeEscaped(ref const(QString) toEscape);

    /**
     * Identical to writeEscaped(), but also escapes quotes.
     */
    /+inline+/ void writeEscapedAttribute(ref const(QString) toEscape);

    /**
     * Writes out @p name.
     */
    /+inline+/ void write(ref const(QXmlName) name);

    /+inline+/ void write(const(char)* const chars);
    /**
     * Encodes and writes out @p content.
     */
    /+inline+/ void write(ref const(QString) content);
