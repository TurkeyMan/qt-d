...
class QXmlName;
class QXmlNamePool;
Q_XMLPATTERNS_EXPORT uint qHash(ref const(QXmlName) name);
...
public:
    ...
    QXmlName(QXmlNamePool &namePool,
             ref const(QString) localName,
             ref const(QString) namespaceURI = QString(),
             ref const(QString) prefix = QString());

    QString namespaceUri(ref const(QXmlNamePool) query) const;
    QString prefix(ref const(QXmlNamePool) query) const;
    QString localName(ref const(QXmlNamePool) query) const;
    QString toClarkName(ref const(QXmlNamePool) query) const;
    bool operator==(ref const(QXmlName) other) const;
    bool operator!=(ref const(QXmlName) other) const;
    QXmlName &operator=(ref const(QXmlName) other);
    bool isNull() const;
    static bool isNCName(ref const(QString) candidate);
    static QXmlName fromClarkName(ref const(QString) clarkName,
                                  ref const(QXmlNamePool) namePool);
    ...
    /+inline+/ bool isLexicallyEqual(ref const(QXmlName) other) const;
    ...
    friend Q_XMLPATTERNS_EXPORT uint qHash(ref const(QXmlName) );
