...
class Q_XMLPATTERNS_EXPORT QXmlSchemaValidator
{
    public:
        QXmlSchemaValidator();
        QXmlSchemaValidator(ref const(QXmlSchema) schema);
        ~QXmlSchemaValidator();

        void setSchema(ref const(QXmlSchema) schema);

        bool validate(ref const(QUrl) source) const;
        bool validate(QIODevice *source, ref const(QUrl) documentUri = QUrl()) const;
        bool validate(ref const(QByteArray) data, ref const(QUrl) documentUri = QUrl()) const;
