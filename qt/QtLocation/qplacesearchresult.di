...
public import qt.QtLocation.QPlace;
...
#define Q_DECLARE_SEARCHRESULT_COPY_CTOR(Class) \
    Class(ref const(QPlaceSearchResult) other);
...
public:
    QPlaceSearchResult();
    QPlaceSearchResult(ref const(QPlaceSearchResult) other);

    /+virtual+/ ~QPlaceSearchResult();

    QPlaceSearchResult &operator=(ref const(QPlaceSearchResult) other);

    bool operator==(ref const(QPlaceSearchResult) other) const;
    bool operator!=(ref const(QPlaceSearchResult) other) const {
        return !(other == *this);
    }
