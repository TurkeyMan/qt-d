...
public import qt.QtLocation.QGeoRouteReply;
...
class Q_LOCATION_EXPORT QGeoRoutingManagerEngine : public QObject
{
    mixin Q_OBJECT;
public:
    QGeoRoutingManagerEngine(ref const(QVariantMap) parameters, QObject *parent = 0);
    /+virtual+/ ~QGeoRoutingManagerEngine();

    QString managerName() const;
    int managerVersion() const;

    /+virtual+/ QGeoRouteReply *calculateRoute(ref const(QGeoRouteRequest) request) = 0;
    /+virtual+/ QGeoRouteReply *updateRoute(ref const(QGeoRoute) route, ref const(QGeoCoordinate) position);
