...
class Q_LOCATION_EXPORT QGeoCodingManagerEngine : public QObject
{
    mixin Q_OBJECT;
public:
    QGeoCodingManagerEngine(ref const(QVariantMap) parameters, QObject *parent = 0);
    /+virtual+/ ~QGeoCodingManagerEngine();

    QString managerName() const;
    int managerVersion() const;

    /+virtual+/ QGeoCodeReply *geocode(ref const(QGeoAddress) address, ref const(QGeoShape) bounds);
    /+virtual+/ QGeoCodeReply *geocode(ref const(QString) address,
                                   int limit,
                                   int offset,
                                   ref const(QGeoShape) bounds);
    /+virtual+/ QGeoCodeReply *reverseGeocode(ref const(QGeoCoordinate) coordinate,
                                          ref const(QGeoShape) bounds);
