...
#define Q_DECLARE_COPY_CTOR(Class, BaseClass) \
    Class(ref const(BaseClass) other);
...
#define Q_IMPLEMENT_COPY_CTOR(Class, BaseClass) \
    Class::Class(ref const(BaseClass) other) : BaseClass() { Class##Private::copyIfPossible(d_ptr, other); }

#define Q_DEFINE_PRIVATE_HELPER(Class, BaseClass, ClassType) \
    BaseClass##Private *clone() const { return new Class##Private(*this); } \
    static void copyIfPossible(QSharedDataPointer<BaseClass##Private> &d_ptr, ref const(BaseClass) other) \
    { \
