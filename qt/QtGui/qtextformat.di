...
Q_GUI_EXPORT QDataStream &operator<<(QDataStream &, ref const(QTextLength) );
Q_GUI_EXPORT QDataStream &operator>>(QDataStream &, QTextLength &);
#endif

#ifndef QT_NO_DEBUG_STREAM
Q_GUI_EXPORT QDebug operator<<(QDebug, ref const(QTextLength) );
#endif
...
public:
    ...
    /+inline+/ bool operator==(ref const(QTextLength) other) const
...
private:
    ...
    friend Q_GUI_EXPORT QDataStream &operator<<(QDataStream &, ref const(QTextLength) );
...
#ifndef QT_NO_DATASTREAM
Q_GUI_EXPORT QDataStream &operator<<(QDataStream &, ref const(QTextFormat) );
Q_GUI_EXPORT QDataStream &operator>>(QDataStream &, QTextFormat &);
#endif

#ifndef QT_NO_DEBUG_STREAM
Q_GUI_EXPORT QDebug operator<<(QDebug, ref const(QTextFormat) );
#endif
...
public:
    ...
    QTextFormat(ref const(QTextFormat) rhs);
    QTextFormat &operator=(ref const(QTextFormat) rhs);
    ~QTextFormat();
    ...
    /+inline+/ bool operator!=(ref const(QTextFormat) rhs) const { return !operator==(rhs); }
    operator QVariant() const;
...
private:
    ...
    friend Q_GUI_EXPORT QDataStream &operator<<(QDataStream &, ref const(QTextFormat) );
...
public:
    ...
    /+inline+/ void setFontFamily(ref const(QString) family)
    ...
    /+inline+/ void setUnderlineColor(ref const(QColor) color)
    ...
    /+inline+/ void setTextOutline(ref const(QPen) pen)
    { setProperty(TextOutline, pen); }
    /+inline+/ QPen textOutline() const
    { return penProperty(TextOutline); }

    /+inline+/ void setToolTip(ref const(QString) tip)
    ...
    /+inline+/ void setAnchorHref(ref const(QString) value)
    { setProperty(AnchorHref, value); }
    /+inline+/ QString anchorHref() const
    { return stringProperty(AnchorHref); }

    /+inline+/ void setAnchorName(ref const(QString) name)
    { setAnchorNames(QStringList(name)); }
    QString anchorName() const;

    /+inline+/ void setAnchorNames(ref const(QStringList) names)
...
protected:
    explicit QTextCharFormat(ref const(QTextFormat) fmt);
...
protected:
    explicit QTextBlockFormat(ref const(QTextFormat) fmt);
    friend class QTextFormat;
};
...
public:
    ...
    /+inline+/ void setNumberPrefix(ref const(QString) numberPrefix);
    /+inline+/ QString numberPrefix() const
    { return stringProperty(ListNumberPrefix); }

    /+inline+/ void setNumberSuffix(ref const(QString) numberSuffix);
    /+inline+/ QString numberSuffix() const
    { return stringProperty(ListNumberSuffix); }

protected:
    explicit QTextListFormat(ref const(QTextFormat) fmt);
    friend class QTextFormat;
};
...
/+inline+/ void QTextListFormat::setNumberPrefix(ref const(QString) np)
{ setProperty(ListNumberPrefix, np); }

/+inline+/ void QTextListFormat::setNumberSuffix(ref const(QString) ns)
...
public:
    ...
    /+inline+/ void setName(ref const(QString) name);
...
protected:
    explicit QTextImageFormat(ref const(QTextFormat) format);
    friend class QTextFormat;
};

Q_DECLARE_SHARED(QTextImageFormat)

/+inline+/ void QTextImageFormat::setName(ref const(QString) aname)
...
//        Absolute
    ...
    /+inline+/ void setBorderBrush(ref const(QBrush) brush)
    ...
    /+inline+/ void setWidth(qreal width);
    /+inline+/ void setWidth(ref const(QTextLength) length)
    { setProperty(FrameWidth, length); }
    /+inline+/ QTextLength width() const
    { return lengthProperty(FrameWidth); }

    /+inline+/ void setHeight(qreal height);
    /+inline+/ void setHeight(ref const(QTextLength) height);
...
protected:
    explicit QTextFrameFormat(ref const(QTextFormat) fmt);
    friend class QTextFormat;
};
...
/+inline+/ void QTextFrameFormat::setHeight(ref const(QTextLength) aheight)
...
public:
    ...
    /+inline+/ void setColumnWidthConstraints(ref const(QVector<QTextLength>) constraints)
...
protected:
    explicit QTextTableFormat(ref const(QTextFormat) fmt);
    friend class QTextFormat;
};
...
protected:
    explicit QTextTableCellFormat(ref const(QTextFormat) fmt);
    friend class QTextFormat;
};
