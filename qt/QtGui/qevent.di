...
public import qt.QtGui.qtouchdevice;
...
{
public:
    QEnterEvent(ref const(QPointF) localPos, ref const(QPointF) windowPos, ref const(QPointF) screenPos);
    ~QEnterEvent();
...
#endif
    ref const(QPointF) localPos() const { return l; }
    ref const(QPointF) windowPos() const { return w; }
    ref const(QPointF) screenPos() const { return s; }
...
{
public:
    QMouseEvent(Type type, ref const(QPointF) localPos, Qt.MouseButton button,
                Qt.MouseButtons buttons, Qt.KeyboardModifiers modifiers);
    QMouseEvent(Type type, ref const(QPointF) localPos, ref const(QPointF) screenPos,
                Qt.MouseButton button, Qt.MouseButtons buttons,
                Qt.KeyboardModifiers modifiers);
    QMouseEvent(Type type, ref const(QPointF) localPos, ref const(QPointF) windowPos, ref const(QPointF) screenPos,
                Qt.MouseButton button, Qt.MouseButtons buttons,
                Qt.KeyboardModifiers modifiers);
    ~QMouseEvent();
...
{
public:
    QHoverEvent(Type type, ref const(QPointF) pos, ref const(QPointF) oldPos, Qt.KeyboardModifiers modifiers = Qt.NoModifier);
    ~QHoverEvent();
    ...
    /+inline+/ ref const(QPointF) posF() const { return p; }
    /+inline+/ ref const(QPointF) oldPosF() const { return op; }
...
{
public:
    QWheelEvent(ref const(QPointF) pos, int delta,
                Qt.MouseButtons buttons, Qt.KeyboardModifiers modifiers,
                Qt.Orientation orient = Qt.Vertical);
    QWheelEvent(ref const(QPointF) pos, ref const(QPointF) globalPos, int delta,
                Qt.MouseButtons buttons, Qt.KeyboardModifiers modifiers,
                Qt.Orientation orient = Qt.Vertical);
    QWheelEvent(ref const(QPointF) pos, ref const(QPointF) globalPos,
...
#endif
    /+inline+/ ref const(QPointF) posF() const { return p; }
    /+inline+/ ref const(QPointF) globalPosF()   const { return g; }
...
public:
    ...
    QTabletEvent(Type t, ref const(QPointF) pos, ref const(QPointF) globalPos,
...
#if QT_DEPRECATED_SINCE(5,0)
    QT_DEPRECATED /+inline+/ ref const(QPointF) hiResGlobalPos() const { return mPos; }
#endif

    /+inline+/ ref const(QPointF) posF() const { return mPos; }
    /+inline+/ ref const(QPointF) globalPosF() const { return mGPos; }
...
{
public:
    QNativeGestureEvent(Qt.NativeGestureType type, ref const(QPointF) localPos, ref const(QPointF) windowPos,
                        ref const(QPointF) screenPos, qreal value, ulong sequenceId, quint64 intArgument);
...
#endif
    ref const(QPointF) localPos() const { return mLocalPos; }
    ref const(QPointF) windowPos() const { return mWindowPos; }
    ref const(QPointF) screenPos() const { return mScreenPos; }
...
{
public:
    QKeyEvent(Type type, int key, Qt.KeyboardModifiers modifiers, ref const(QString) text = QString(),
              bool autorep = false, ushort count = 1);
    QKeyEvent(Type type, int key, Qt.KeyboardModifiers modifiers,
              quint32 nativeScanCode, quint32 nativeVirtualKey, quint32 nativeModifiers,
              ref const(QString) text = QString(), bool autorep = false, ushort count = 1);
    ~QKeyEvent();
    ...
    static /+inline+/ QKeyEvent *createExtendedKeyEvent(Type type, int key, Qt.KeyboardModifiers modifiers,
                                             ...
                                             ref const(QString) text = QString(), bool autorep = false,
                                             ushort count = 1)
    {
...
{
public:
    explicit QPaintEvent(ref const(QRegion) paintRegion);
    explicit QPaintEvent(ref const(QRect) paintRect);
    ~QPaintEvent();

    /+inline+/ ref const(QRect) rect() const { return m_rect; }
    /+inline+/ ref const(QRegion) region() const { return m_region; }
...
{
public:
    QMoveEvent(ref const(QPoint) pos, ref const(QPoint) oldPos);
    ~QMoveEvent();

    /+inline+/ ref const(QPoint) pos() const { return p; }
    /+inline+/ ref const(QPoint) oldPos() const { return oldp;}
protected:
    QPoint p, oldp;
...
{
public:
    explicit QExposeEvent(ref const(QRegion) rgn);
    ~QExposeEvent();

    /+inline+/ ref const(QRegion) region() const { return rgn; }
...
{
public:
    QResizeEvent(ref const(QSize) size, ref const(QSize) oldSize);
    ~QResizeEvent();

    /+inline+/ ref const(QSize) size() const { return s; }
    /+inline+/ ref const(QSize) oldSize()const { return olds;}
protected:
    QSize s, olds;
    friend class QApplication;
};
...
public:
    ...
    QContextMenuEvent(Reason reason, ref const(QPoint) pos, ref const(QPoint) globalPos,
                      Qt.KeyboardModifiers modifiers);
    QContextMenuEvent(Reason reason, ref const(QPoint) pos, ref const(QPoint) globalPos);
    QContextMenuEvent(Reason reason, ref const(QPoint) pos);
    ~QContextMenuEvent();
    ...
    /+inline+/ ref const(QPoint) pos() const { return p; }
    /+inline+/ ref const(QPoint) globalPos() const { return gp; }
...
public:
    ...
    QInputMethodEvent();
    QInputMethodEvent(ref const(QString) preeditText, ref const(QList<Attribute>) attributes);
    void setCommitString(ref const(QString) commitString, int replaceFrom = 0, int replaceLength = 0);
    /+inline+/ ref const(QList<Attribute>) attributes() const { return attrs; }
    /+inline+/ ref const(QString) preeditString() const { return preedit; }

    /+inline+/ ref const(QString) commitString() const { return commit; }
    /+inline+/ int replacementStart() const { return replace_from; }
    /+inline+/ int replacementLength() const { return replace_length; }

    QInputMethodEvent(ref const(QInputMethodEvent) other);
...
public:
    ...
    void setValue(Qt.InputMethodQuery query, ref const(QVariant) value);
...
{
public:
    QDropEvent(ref const(QPointF) pos, Qt.DropActions actions, const(QMimeData)* data,
...
{
public:
    QDragMoveEvent(ref const(QPoint) pos, Qt.DropActions actions, const(QMimeData)* data,
    ...
    /+inline+/ void accept(ref const(QRect)  r) { accept(); rect = r; }
    /+inline+/ void ignore(ref const(QRect)  r) { ignore(); rect = r; }
...
{
public:
    QDragEnterEvent(ref const(QPoint) pos, Qt.DropActions actions, const(QMimeData)* data,
                    Qt.MouseButtons buttons, Qt.KeyboardModifiers modifiers);
    ~QDragEnterEvent();
};
...
{
public:
    QHelpEvent(Type type, ref const(QPoint) pos, ref const(QPoint) globalPos);
    ~QHelpEvent();
...
{
public:
    explicit QStatusTipEvent(ref const(QString) tip);
    ~QStatusTipEvent();
...
{
public:
    explicit QWhatsThisClickedEvent(ref const(QString) href);
...
{
public:
    explicit QFileOpenEvent(ref const(QString) file);
    explicit QFileOpenEvent(ref const(QUrl) url);
    ~QFileOpenEvent();
...
{
public:
    QShortcutEvent(ref const(QKeySequence) key, int id, bool ambiguous = false);
    ~QShortcutEvent();

    /+inline+/ ref const(QKeySequence) key() const { return sequence; }
...
public:
    ...
    public:
        ...
        TouchPoint(ref const(TouchPoint) other);
        ...
        TouchPoint &operator=(ref const(TouchPoint) other)
        ...
        void setSceneRect(ref const(QRectF) sceneRect);
        void setScreenRect(ref const(QRectF) screenRect);
    ...
    explicit QTouchEvent(QEvent::Type eventType,
                         ...
                         ref const(QList<QTouchEvent::TouchPoint>) touchPoints = QList<QTouchEvent::TouchPoint>());
    ~QTouchEvent();
    ...
    /+inline+/ ref const(QList<QTouchEvent::TouchPoint>) touchPoints() const { return _touchPoints; }
    ...
    /+inline+/ void setTouchPoints(ref const(QList<QTouchEvent::TouchPoint>) atouchPoints) { _touchPoints = atouchPoints; }
...
#ifndef QT_NO_DEBUG_STREAM
Q_GUI_EXPORT QDebug operator<<(QDebug, ref const(QTouchEvent::TouchPoint) );
#endif

class Q_GUI_EXPORT QScrollPrepareEvent : public QEvent
{
public:
    explicit QScrollPrepareEvent(ref const(QPointF) startPos);
    ~QScrollPrepareEvent();
    ...
    void setViewportSize(ref const(QSizeF) size);
    void setContentPosRange(ref const(QRectF) rect);
    void setContentPos(ref const(QPointF) pos);
...
public:
    ...
    QScrollEvent(ref const(QPointF) contentPos, ref const(QPointF) overshoot, ScrollState scrollState);
    ~QScrollEvent();
