...
public:
    ...
    /+virtual+/ void draw(QPainter *painter, ref const(PaintContext) context) = 0;
    /+virtual+/ int hitTest(ref const(QPointF) point, Qt.HitTestAccuracy accuracy) const = 0;
    QString anchorAt(ref const(QPointF) pos) const;

    /+virtual+/ int pageCount() const = 0;
    /+virtual+/ QSizeF documentSize() const = 0;

    /+virtual+/ QRectF frameBoundingRect(QTextFrame *frame) const = 0;
    /+virtual+/ QRectF blockBoundingRect(ref const(QTextBlock) block) const = 0;
...
Q_SIGNALS:
    void update(ref const(QRectF)  = QRectF(0., 0., 1000000000., 1000000000.));
    void updateBlock(ref const(QTextBlock) block);
    void documentSizeChanged(ref const(QSizeF) newSize);
...
public:
    ...
    /+virtual+/ QSizeF intrinsicSize(QTextDocument *doc, int posInDocument, ref const(QTextFormat) format) = 0;
    /+virtual+/ void drawObject(QPainter *painter, ref const(QRectF) rect, QTextDocument *doc, int posInDocument, ref const(QTextFormat) format) = 0;
};
