...
Q_GUI_EXPORT QDebug operator<<(QDebug, ref const(QColor) );
#endif
#ifndef QT_NO_DATASTREAM
Q_GUI_EXPORT QDataStream &operator<<(QDataStream &, ref const(QColor) );
...
public:
    ...
    QColor(QRgb rgb);
    QColor(ref const(QString) name);
    QColor(const(char)* name);
    QColor(ref const(QColor) color);
    QColor(Spec spec);
    ...
    QColor &operator=(ref const(QColor) );
    QColor &operator=(Qt.GlobalColor color);

    bool operator==(ref const(QColor) c) const;
    bool operator!=(ref const(QColor) c) const;
...
private:
    ...
    void invalidate();
    bool setColorFromString(ref const(QString) name);
...
#ifndef QT_NO_DATASTREAM
    friend Q_GUI_EXPORT QDataStream &operator<<(QDataStream &, ref const(QColor) );
...
/+inline+/ QColor::QColor(ref const(QString) aname)
{ setNamedColor(aname); }

/+inline+/ QColor::QColor(ref const(QColor) acolor)
    : cspec(acolor.cspec)
