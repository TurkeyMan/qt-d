...
public:
    QBitmap();
    QBitmap(ref const(QPixmap) );
    QBitmap(int w, int h);
    explicit QBitmap(ref const(QSize) );
    explicit QBitmap(ref const(QString) fileName, const(char)* format=0);
    ~QBitmap();

    QBitmap &operator=(ref const(QPixmap) );
    /+inline+/ void swap(QBitmap &other) { QPixmap::swap(other); } // prevent QBitmap<->QPixmap swaps
    operator QVariant() const;

    /+inline+/ void clear() { fill(Qt.color0); }

    static QBitmap fromImage(ref const(QImage) image, Qt.ImageConversionFlags flags = Qt.AutoColor);
