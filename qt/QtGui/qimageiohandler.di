...
public:
    ...
    void setFormat(ref const(QByteArray) format) const;
...
public:
    ...
    /+virtual+/ Capabilities capabilities(QIODevice *device, ref const(QByteArray) format) const = 0;
    /+virtual+/ QImageIOHandler *create(QIODevice *device, ref const(QByteArray) format = QByteArray()) const = 0;
};
