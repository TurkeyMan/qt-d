...
public:
    ...
    explicit QPageSize(ref const(QSize) pointSize,
                       ref const(QString) name = QString(),
                       SizeMatchPolicy matchPolicy = FuzzyMatch);
    explicit QPageSize(ref const(QSizeF) size, Unit units,
                       ref const(QString) name = QString(),
                       SizeMatchPolicy matchPolicy = FuzzyMatch);
    QPageSize(ref const(QPageSize) other);
    ~QPageSize();

    QPageSize &operator=(ref const(QPageSize) other);
    ...
    friend Q_GUI_EXPORT bool operator==(ref const(QPageSize) lhs, ref const(QPageSize) rhs);
...
private:
    ...
    QPageSize(ref const(QString) key, ref const(QSize) pointSize, ref const(QString) name);
    QPageSize(int windowsId, ref const(QSize) pointSize, ref const(QString) name);
...
/+inline+/ bool operator!=(ref const(QPageSize) lhs, ref const(QPageSize) rhs)
{ return !operator==(lhs, rhs); }

#ifndef QT_NO_DEBUG_STREAM
Q_GUI_EXPORT QDebug operator<<(QDebug dbg, ref const(QPageSize) pageSize);
#endif
