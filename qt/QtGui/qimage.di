...
public import qt.QtGui.qrgb;
public import qt.QtGui.qpixelformat;
...
public:
    ...
    bool operator< (ref const(QImageTextKeyLang) other) const
        { return key < other.key || (key==other.key && lang < other.lang); }
    bool operator== (ref const(QImageTextKeyLang) other) const
        { return key==other.key && lang==other.lang; }
    /+inline+/ bool operator!= (ref const(QImageTextKeyLang) other) const
...
public:
    ...
    QImage();
    QImage(ref const(QSize) size, Format format);
...
#endif
    explicit QImage(ref const(QString) fileName, const(char)* format = 0);

    QImage(ref const(QImage) );
    ...
    QImage &operator=(ref const(QImage) );
#ifdef Q_COMPILER_RVALUE_REFS
    ...
    QImage copy(ref const(QRect) rect = QRect()) const;
    ...
    QImage convertToFormat(Format f, Qt.ImageConversionFlags flags = Qt.AutoColor) const Q_REQUIRED_RESULT;
#endif
    QImage convertToFormat(Format f, ref const(QVector<QRgb>) colorTable, Qt.ImageConversionFlags flags = Qt.AutoColor) const Q_REQUIRED_RESULT;
    ...
    bool valid(ref const(QPoint) pt) const;

    int pixelIndex(int x, int y) const;
    int pixelIndex(ref const(QPoint) pt) const;

    QRgb pixel(int x, int y) const;
    QRgb pixel(ref const(QPoint) pt) const;

    void setPixel(int x, int y, uint index_or_rgb);
    void setPixel(ref const(QPoint) pt, uint index_or_rgb);
    ...
    QImage scaled(ref const(QSize) s, Qt.AspectRatioMode aspectMode = Qt.IgnoreAspectRatio,
                 Qt.TransformationMode mode = Qt.FastTransformation) const;
    QImage scaledToWidth(int w, Qt.TransformationMode mode = Qt.FastTransformation) const;
    QImage scaledToHeight(int h, Qt.TransformationMode mode = Qt.FastTransformation) const;
    QImage transformed(ref const(QMatrix) matrix, Qt.TransformationMode mode = Qt.FastTransformation) const;
    static QMatrix trueMatrix(ref const(QMatrix) , int w, int h);
    QImage transformed(ref const(QTransform) matrix, Qt.TransformationMode mode = Qt.FastTransformation) const;
    ...
    bool load(ref const(QString) fileName, const(char)* format=0);
    bool loadFromData(const(uchar)* buf, int len, const(char)* format = 0);
    /+inline+/ bool loadFromData(ref const(QByteArray) data, const(char)* aformat=0)
        { return loadFromData(reinterpret_cast<const(uchar)* >(data.constData()), data.size(), aformat); }

    bool save(ref const(QString) fileName, const(char)* format=0, int quality=-1) const;
    bool save(QIODevice *device, const(char)* format=0, int quality=-1) const;

    static QImage fromData(const(uchar)* data, int size, const(char)* format = 0);
    /+inline+/ static QImage fromData(ref const(QByteArray) data, const(char)* format = 0)
    ...
    QString text(ref const(QString) key = QString()) const;
    ...
    QT_DEPRECATED /+inline+/ QString text(ref const(QImageTextKeyLang)) const;
    QT_DEPRECATED /+inline+/ void setText(const(char)* key, const(char)* lang, ref const(QString));
#endif
...
/+inline+/ bool QImage::valid(ref const(QPoint) pt) const { return valid(pt.x(), pt.y()); }
/+inline+/ int QImage::pixelIndex(ref const(QPoint) pt) const { return pixelIndex(pt.x(), pt.y());}
/+inline+/ QRgb QImage::pixel(ref const(QPoint) pt) const { return pixel(pt.x(), pt.y()); }
/+inline+/ void QImage::setPixel(ref const(QPoint) pt, uint index_or_rgb) { setPixel(pt.x(), pt.y(), index_or_rgb); }
...
/+inline+/ QString QImage::text(ref const(QImageTextKeyLang)kl) const
{
    if (!d)
        return QString();
...
/+inline+/ void QImage::setText(const(char)* key, const(char)* lang, ref const(QString) s)
{
    if (!d)
        return;
    detach();
...
Q_GUI_EXPORT QDataStream &operator<<(QDataStream &, ref const(QImage) );
Q_GUI_EXPORT QDataStream &operator>>(QDataStream &, QImage &);
#endif

#ifndef QT_NO_DEBUG_STREAM
Q_GUI_EXPORT QDebug operator<<(QDebug, ref const(QImage) );
#endif
