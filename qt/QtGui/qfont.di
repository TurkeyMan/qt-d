...
public:
    ...
    QFont();
    QFont(ref const(QString) family, int pointSize = -1, int weight = -1, bool italic = false);
    QFont(ref const(QFont) , QPaintDevice *pd);
    QFont(ref const(QFont) );
    ~QFont();
    ...
    QFont &operator=(ref const(QFont) );
    ...
    static QT_DEPRECATED void removeSubstitution(ref const(QString) family) { removeSubstitutions(family); }
#endif
...
private:
    ...
    friend Q_GUI_EXPORT QDataStream &operator<<(QDataStream &, ref const(QFont) );
...
Q_GUI_EXPORT uint qHash(ref const(QFont) font, uint seed = 0) Q_DECL_NOTHROW;
...
#ifndef QT_NO_DATASTREAM
Q_GUI_EXPORT QDataStream &operator<<(QDataStream &, ref const(QFont) );
Q_GUI_EXPORT QDataStream &operator>>(QDataStream &, QFont &);
#endif

#ifndef QT_NO_DEBUG_STREAM
Q_GUI_EXPORT QDebug operator<<(QDebug, ref const(QFont) );
#endif
