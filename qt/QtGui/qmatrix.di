...
public:
    ...
    QMatrix(qreal m11, qreal m12, qreal m21, qreal m22,
            qreal dx, qreal dy);
    QMatrix(ref const(QMatrix) matrix);
    ...
    QMatrix &operator*=(ref const(QMatrix) );
...
/+inline+/ QPoint operator*(ref const(QPoint) p, ref const(QMatrix) m)
{ return m.map(p); }
/+inline+/ QPointF operator*(ref const(QPointF) p, ref const(QMatrix) m)
{ return m.map(p); }
/+inline+/ QLineF operator*(ref const(QLineF) l, ref const(QMatrix) m)
{ return m.map(l); }
/+inline+/ QLine operator*(ref const(QLine) l, ref const(QMatrix) m)
{ return m.map(l); }
/+inline+/ QPolygon operator *(ref const(QPolygon) a, ref const(QMatrix) m)
{ return m.map(a); }
/+inline+/ QPolygonF operator *(ref const(QPolygonF) a, ref const(QMatrix) m)
{ return m.map(a); }
/+inline+/ QRegion operator *(ref const(QRegion) r, ref const(QMatrix) m)
{ return m.map(r); }
...
/+inline+/ bool qFuzzyCompare(ref const(QMatrix) m1, ref const(QMatrix) m2)
{
...
Q_GUI_EXPORT QDataStream &operator<<(QDataStream &, ref const(QMatrix) );
Q_GUI_EXPORT QDataStream &operator>>(QDataStream &, QMatrix &);
#endif

#ifndef QT_NO_DEBUG_STREAM
Q_GUI_EXPORT QDebug operator<<(QDebug, ref const(QMatrix) );
#endif
