...
public:
    ...
    QPixmap(int w, int h);
    explicit QPixmap(ref const(QSize) );
    QPixmap(ref const(QString) fileName, const(char)* format = 0, Qt.ImageConversionFlags flags = Qt.AutoColor);
#ifndef QT_NO_IMAGEFORMAT_XPM
    explicit QPixmap(const(char)*  const xpm[]);
#endif
    QPixmap(ref const(QPixmap) );
    ~QPixmap();

    QPixmap &operator=(ref const(QPixmap) );
...
#endif
    QBitmap createMaskFromColor(ref const(QColor) maskColor, Qt.MaskMode mode = Qt.MaskInColor) const;
    ...
    QPixmap scaled(ref const(QSize) s, Qt.AspectRatioMode aspectMode = Qt.IgnoreAspectRatio,
                   Qt.TransformationMode mode = Qt.FastTransformation) const;
    QPixmap scaledToWidth(int w, Qt.TransformationMode mode = Qt.FastTransformation) const;
    QPixmap scaledToHeight(int h, Qt.TransformationMode mode = Qt.FastTransformation) const;
    QPixmap transformed(ref const(QMatrix) , Qt.TransformationMode mode = Qt.FastTransformation) const;
    static QMatrix trueMatrix(ref const(QMatrix) m, int w, int h);
    QPixmap transformed(ref const(QTransform) , Qt.TransformationMode mode = Qt.FastTransformation) const;
    static QTransform trueMatrix(ref const(QTransform) m, int w, int h);

    QImage toImage() const;
    static QPixmap fromImage(ref const(QImage) image, Qt.ImageConversionFlags flags = Qt.AutoColor);
    ...
    bool load(ref const(QString) fileName, const(char)* format = 0, Qt.ImageConversionFlags flags = Qt.AutoColor);
    bool loadFromData(const(uchar)* buf, uint len, const(char)* format = 0, Qt.ImageConversionFlags flags = Qt.AutoColor);
    /+inline+/ bool loadFromData(ref const(QByteArray) data, const(char)* format = 0, Qt.ImageConversionFlags flags = Qt.AutoColor);
    bool save(ref const(QString) fileName, const(char)* format = 0, int quality = -1) const;
    bool save(QIODevice* device, const(char)* format = 0, int quality = -1) const;

    bool convertFromImage(ref const(QImage) img, Qt.ImageConversionFlags flags = Qt.AutoColor);

    /+inline+/ QPixmap copy(int x, int y, int width, int height) const;
    QPixmap copy(ref const(QRect) rect = QRect()) const;

    /+inline+/ void scroll(int dx, int dy, int x, int y, int width, int height, QRegion *exposed = 0);
    void scroll(int dx, int dy, ref const(QRect) rect, QRegion *exposed = 0);
    ...
    QT_DEPRECATED /+inline+/ void setAlphaChannel(ref const(QPixmap) );
#endif
...
private:
    ...
    QPixmap(ref const(QSize) s, int type);
...
/+inline+/ bool QPixmap::loadFromData(ref const(QByteArray) buf, const(char)* format,
...
/+inline+/ void QPixmap::setAlphaChannel(ref const(QPixmap) p)
{
...
#if !defined(QT_NO_DATASTREAM)
Q_GUI_EXPORT QDataStream &operator<<(QDataStream &, ref const(QPixmap) );
Q_GUI_EXPORT QDataStream &operator>>(QDataStream &, QPixmap &);
#endif

#ifndef QT_NO_DEBUG_STREAM
Q_GUI_EXPORT QDebug operator<<(QDebug, ref const(QPixmap) );
#endif
