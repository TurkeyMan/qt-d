...
public:
    ...
    bool save(ref const(QString) fileName, const(char)* format = 0);

    QRect boundingRect() const;
    void setBoundingRect(ref const(QRect) r);

    QPicture& operator=(ref const(QPicture) p);
    ...
    friend Q_GUI_EXPORT QDataStream &operator<<(QDataStream &in, ref const(QPicture) p);
    friend Q_GUI_EXPORT QDataStream &operator>>(QDataStream &in, QPicture &p);

    static const(char)* pictureFormat(ref const(QString) fileName);
...
public:
    ...
    QPictureIO(ref const(QString) fileName, const(char)* format);
    ~QPictureIO();
...
Q_GUI_EXPORT QDataStream &operator<<(QDataStream &, ref const(QPicture) );
