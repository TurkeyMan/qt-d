...
public import qt.QtGui.qopenglcontext;
...
public:
    ...
    QOpenGLDebugMessage();
    QOpenGLDebugMessage(ref const(QOpenGLDebugMessage) debugMessage);
    ~QOpenGLDebugMessage();

    QOpenGLDebugMessage &operator=(ref const(QOpenGLDebugMessage) debugMessage);
    ...
    static QOpenGLDebugMessage createApplicationMessage(ref const(QString) text,
                                                        GLuint id = 0,
                                                        Severity severity = NotificationSeverity,
                                                        Type type = OtherType);
    static QOpenGLDebugMessage createThirdPartyMessage(ref const(QString) text,
                                                       GLuint id = 0,
                                                       Severity severity = NotificationSeverity,
                                                       Type type = OtherType);

    bool operator==(ref const(QOpenGLDebugMessage) debugMessage) const;
    /+inline+/ bool operator!=(ref const(QOpenGLDebugMessage) debugMessage) const { return !operator==(debugMessage); }
...
Q_GUI_EXPORT QDebug operator<<(QDebug debug, ref const(QOpenGLDebugMessage) message);
...
class Q_GUI_EXPORT QOpenGLDebugLogger : public QObject
{
    mixin Q_OBJECT;
    Q_ENUMS(LoggingMode)
    mixin Q_PROPERTY!(LoggingMode, "loggingMode", "READ", "loggingMode");
...
public:
    ...
    void pushGroup(ref const(QString) name,
                   GLuint id = 0,
    ...
    void enableMessages(ref const(QVector<GLuint>) ids,
    ...
    void disableMessages(ref const(QVector<GLuint>) ids,
                         QOpenGLDebugMessage::Sources sources = QOpenGLDebugMessage::AnySource,
                         QOpenGLDebugMessage::Types types = QOpenGLDebugMessage::AnyType);
...
public Q_SLOTS:
    void logMessage(ref const(QOpenGLDebugMessage) debugMessage);
    void startLogging(LoggingMode loggingMode = AsynchronousLogging);
    void stopLogging();

Q_SIGNALS:
    void messageLogged(ref const(QOpenGLDebugMessage) debugMessage);
