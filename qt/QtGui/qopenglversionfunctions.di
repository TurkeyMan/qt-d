...
/+inline+/ uint qHash(ref const(QOpenGLVersionStatus) v, uint seed = 0) Q_DECL_NOTHROW
{
    return qHash(static_cast<int>(v.status * 1000)
               + v.version.first * 100 + v.version.second * 10, seed);
}

Q_DECL_CONSTEXPR /+inline+/ bool operator==(ref const(QOpenGLVersionStatus) lhs, ref const(QOpenGLVersionStatus) rhs)
{
...
public:
    ...
    static QOpenGLVersionFunctionsBackend *functionsBackend(QOpenGLContext *context,
                                                            ref const(QOpenGLVersionStatus) v);
    static void insertFunctionsBackend(QOpenGLContext *context,
                                       ref const(QOpenGLVersionStatus) v,
                                       QOpenGLVersionFunctionsBackend *backend);
    static void removeFunctionsBackend(QOpenGLContext *context, ref const(QOpenGLVersionStatus) v);
