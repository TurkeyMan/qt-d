...
public import qt.QtConcurrent.qtconcurrentiteratekernel;
...
public:
    FilterKernel(Sequence &_sequence, KeepFunctor _keep, ReduceFunctor _reduce)
        : IterateKernelType(const_cast<ref const(Sequence) >(_sequence).begin(), const_cast<ref const(Sequence) >(_sequence).end()), reducedResult(),
          sequence(_sequence),
          keep(_keep),
          reduce(_reduce),
...
startFiltered(ref const(Sequence) sequence, KeepFunctor functor)
{
...
/+inline+/ ThreadEngineStarter<ResultType> startFilteredReduced(ref const(Sequence)  sequence,
