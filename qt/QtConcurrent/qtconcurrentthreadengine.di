...
public import qt.QtCore.qexception;
public import qt.QtCore.qwaitcondition;
public import qt.QtCore.qatomic;
public import qt.QtCore.qsemaphore;
...
private:
    ...
    void handleException(ref const(QException) exception);
#endif
protected:
...
public:
    ...
    void reportResults(ref const(QVector<T>) _result, int index = -1, int count = -1)
    {
        if (futureInterface)
...
public:
    ...
    /+inline+/ ThreadEngineStarterBase(ref const(ThreadEngineStarterBase) other)
...
/+inline+/ ThreadEngineStarter<typename ThreadEngine::ResultType> startThreadEngine(ThreadEngine)(ThreadEngine *threadEngine)
{
