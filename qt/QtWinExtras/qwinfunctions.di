...
namespace QtWin
{
    ...
    Q_WINEXTRAS_EXPORT HBITMAP createMask(ref const(QBitmap) bitmap);
    Q_WINEXTRAS_EXPORT HBITMAP toHBITMAP(ref const(QPixmap) p, HBitmapFormat format = HBitmapNoAlpha);
    Q_WINEXTRAS_EXPORT QPixmap fromHBITMAP(HBITMAP bitmap, HBitmapFormat format = HBitmapNoAlpha);
    Q_WINEXTRAS_EXPORT HICON toHICON(ref const(QPixmap) p);
    ...
    Q_WINEXTRAS_EXPORT HRGN toHRGN(ref const(QRegion) region);
    ...
    Q_WINEXTRAS_EXPORT void extendFrameIntoClientArea(QWindow *window, ref const(QMargins) margins);
    Q_WINEXTRAS_EXPORT void resetExtendedFrame(QWindow *window);

    Q_WINEXTRAS_EXPORT void enableBlurBehindWindow(QWindow *window, ref const(QRegion) region);
    ...
    Q_WINEXTRAS_EXPORT void setCurrentProcessExplicitAppUserModelID(ref const(QString) id);
    ...
    /+inline+/ void extendFrameIntoClientArea(QWidget *window, ref const(QMargins) margins)
    {
        window->createWinId();
    ...
    /+inline+/ void enableBlurBehindWindow(QWidget *window, ref const(QRegion) region)
    {
        window->createWinId();
