...
public import qt.QtMultimedia.qcameraexposure;
public import qt.QtMultimedia.qcamerafocus;
public import qt.QtMultimedia.qcameraimageprocessing;
...
{
    mixin Q_OBJECT;
    mixin Q_PROPERTY!(QCamera::State, "state", "READ", "state", "NOTIFY", "stateChanged");
    mixin Q_PROPERTY!(QCamera::Status, "status", "READ", "status", "NOTIFY", "statusChanged");
    mixin Q_PROPERTY!(QCamera::CaptureModes, "captureMode", "READ", "captureMode", "WRITE", "setCaptureMode", "NOTIFY", "captureModeChanged");
    mixin Q_PROPERTY!(QCamera::LockStatus, "lockStatus", "READ", "lockStatus", "NOTIFY", "lockStatusChanged");
...
public:
    ...
    QCamera(ref const(QByteArray) deviceName, QObject *parent = 0);
    QCamera(ref const(QCameraInfo) cameraInfo, QObject *parent = 0);
    QCamera(QCamera::Position position, QObject *parent = 0);
    ~QCamera();

#if QT_DEPRECATED_SINCE(5, 3)
    QT_DEPRECATED static QList<QByteArray> availableDevices();
    QT_DEPRECATED static QString deviceDescription(ref const(QByteArray) device);
#endif
