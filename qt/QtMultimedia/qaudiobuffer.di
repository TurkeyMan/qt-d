...
public:
    ...
    QAudioBuffer(ref const(QAudioBuffer) other);
    QAudioBuffer(ref const(QByteArray) data, ref const(QAudioFormat) format, qint64 startTime = -1);
    QAudioBuffer(int numFrames, ref const(QAudioFormat) format, qint64 startTime = -1); // Initialized to empty

    QAudioBuffer& operator=(ref const(QAudioBuffer) other);
    ...
    template <typename T> struct StereoFrame {
        ...
        StereoFrame& operator=(ref const(StereoFrame) other)
        {
