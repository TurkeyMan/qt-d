...
public import QtQml.qqmlprivate;
public import QtQml.qqmlparserstatus;
public import QtQml.qqmlpropertyvaluesource;
public import QtQml.qqmllist;
...
QT_BEGIN_NAMESPACE \
template <> \
extern(C++) class QQmlTypeInfo<TYPE > \
{ \
public: \
    enum { \
...
extern(C++) class QQmlPropertyValueInterceptor;
...
int Q_QML_EXPORT qmlRegisterTypeNotAvailable(const(char)* uri, int versionMajor, int versionMinor, const(char)* qmlName, ref const(QString) message);

template<typename T>
int qmlRegisterUncreatableType(const(char)* uri, int versionMajor, int versionMinor, const(char)* qmlName, ref const(QString) reason)
{
    QML_GETTYPENAMES
...
int qmlRegisterExtendedUncreatableType(const(char)* uri, int versionMajor, int versionMinor, const(char)* qmlName, ref const(QString) reason)
{
    QML_GETTYPENAMES
...
/+inline+/ int qmlRegisterSingletonType(ref const(QUrl) url, const(char)* uri, int versionMajor, int versionMinor, const(char)* qmlName)
{
    if (url.isRelative()) {
...
/+inline+/ int qmlRegisterType(ref const(QUrl) url, const(char)* uri, int versionMajor, int versionMinor, const(char)* qmlName)
{
    if (url.isRelative()) {
        // User input check must go here, because QQmlPrivate::qmlregister is also used internally for composite types
