...
public:
    QSslKey();
    QSslKey(ref const(QByteArray) encoded, QSsl::KeyAlgorithm algorithm,
    ...
    QSslKey(ref const(QSslKey) other);
    ~QSslKey();
    QSslKey &operator=(ref const(QSslKey) other);
    ...
    QByteArray toPem(ref const(QByteArray) passPhrase = QByteArray()) const;
    QByteArray toDer(ref const(QByteArray) passPhrase = QByteArray()) const;

    Qt.HANDLE handle() const;

    bool operator==(ref const(QSslKey) key) const;
    /+inline+/ bool operator!=(ref const(QSslKey) key) const { return !operator==(key); }
...
class QDebug;
Q_NETWORK_EXPORT QDebug operator<<(QDebug debug, ref const(QSslKey) key);
#endif
