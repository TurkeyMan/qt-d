...
public import qt.QtCore.qcryptographichash;
...
public import qt.QtNetwork.qssl;
...
Q_NETWORK_EXPORT uint qHash(ref const(QSslCertificate) key, uint seed = 0) Q_DECL_NOTHROW;
...
public:
    ...
    explicit QSslCertificate(ref const(QByteArray) data = QByteArray(), QSsl::EncodingFormat format = QSsl::Pem);
    QSslCertificate(ref const(QSslCertificate) other);
    ~QSslCertificate();
    QSslCertificate &operator=(ref const(QSslCertificate) other);

    /+inline+/ void swap(QSslCertificate &other)
    { qSwap(d, other.d); }

    bool operator==(ref const(QSslCertificate) other) const;
    /+inline+/ bool operator!=(ref const(QSslCertificate) other) const { return !operator==(other); }
    ...
    static QList<QSslCertificate> fromPath(
        ref const(QString) path, QSsl::EncodingFormat format = QSsl::Pem,
        QRegExp::PatternSyntax syntax = QRegExp::FixedString);
    static QList<QSslCertificate> fromDevice(
        QIODevice *device, QSsl::EncodingFormat format = QSsl::Pem);
    static QList<QSslCertificate> fromData(
        ref const(QByteArray) data, QSsl::EncodingFormat format = QSsl::Pem);

    static QList<QSslError> verify(QList<QSslCertificate> certificateChain, ref const(QString) hostName = QString());
...
private:
    ...
    friend Q_NETWORK_EXPORT uint qHash(ref const(QSslCertificate) key, uint seed) Q_DECL_NOTHROW;
};
Q_DECLARE_SHARED(QSslCertificate)

#ifndef QT_NO_DEBUG_STREAM
class QDebug;
Q_NETWORK_EXPORT QDebug operator<<(QDebug debug, ref const(QSslCertificate) certificate);
