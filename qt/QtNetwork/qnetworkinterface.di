...
public:
    ...
    QNetworkAddressEntry(ref const(QNetworkAddressEntry) other);
    QNetworkAddressEntry &operator=(ref const(QNetworkAddressEntry) other);
    ~QNetworkAddressEntry();

    void swap(QNetworkAddressEntry &other) { qSwap(d, other.d); }

    bool operator==(ref const(QNetworkAddressEntry) other) const;
    /+inline+/ bool operator!=(ref const(QNetworkAddressEntry) other) const
    { return !(*this == other); }

    QHostAddress ip() const;
    void setIp(ref const(QHostAddress) newIp);

    QHostAddress netmask() const;
    void setNetmask(ref const(QHostAddress) newNetmask);
    int prefixLength() const;
    void setPrefixLength(int length);

    QHostAddress broadcast() const;
    void setBroadcast(ref const(QHostAddress) newBroadcast);
...
public:
    ...
    QNetworkInterface();
    QNetworkInterface(ref const(QNetworkInterface) other);
    QNetworkInterface &operator=(ref const(QNetworkInterface) other);
    ~QNetworkInterface();
    ...
    static QNetworkInterface interfaceFromName(ref const(QString) name);
...
Q_NETWORK_EXPORT QDebug operator<<(QDebug debug, ref const(QNetworkInterface) networkInterface);
#endif
