...
public:
    ...
    void setOptionsPages(ref const(QList<QDesignerOptionsPageInterface*>) optionsPages);

    QObjectList pluginInstances() const;

    static QIcon createIcon(ref const(QString) name);
...
private:
    QDesignerFormEditorInterface(ref const(QDesignerFormEditorInterface) other);
    void operator = (ref const(QDesignerFormEditorInterface) other);
};
