...
public import QtGui.qopenglshaderprogram;

extern(C++) class QSGMaterial;
extern(C++) class QSGMaterialShaderPrivate;

namespace QSGBatchRenderer {
    extern(C++) class ShaderManager;
}

extern(C++) class Q_QUICK_EXPORT QSGMaterialShader
{
public:
    extern(C++) class Q_QUICK_EXPORT RenderState {
    public:
        enum DirtyState
        {
            DirtyMatrix         = 0x0001,
    ...
    /+virtual+/ void updateState(ref const(RenderState) state, QSGMaterial *newMaterial, QSGMaterial *oldMaterial);
...
protected:
    ...
    friend extern(C++) class QSGRenderContext;
    friend extern(C++) class QSGBatchRenderer::ShaderManager;

    void setShaderSourceFile(QOpenGLShader::ShaderType type, ref const(QString) sourceFile);
    void setShaderSourceFiles(QOpenGLShader::ShaderType type, ref const(QStringList) sourceFiles);
...
extern(C++) class Q_QUICK_EXPORT QSGMaterial
{
public:
    enum Flag {
        Blending            = 0x0001,
