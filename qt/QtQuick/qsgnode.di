...
public import QtQuick.qsggeometry;
...
extern(C++) class QSGRenderer;

extern(C++) class QSGNode;
extern(C++) class QSGRootNode;
extern(C++) class QSGGeometryNode;
extern(C++) class QSGTransformNode;
extern(C++) class QSGClipNode;
extern(C++) class QSGNodePrivate;
extern(C++) class QSGBasicGeometryNodePrivate;
extern(C++) class QSGGeometryNodePrivate;

namespace QSGBatchRenderer {
    extern(C++) class Renderer;
    extern(C++) class Updater;
}

extern(C++) class Q_QUICK_EXPORT QSGNode
{
public:
    enum NodeType {
        BasicNodeType,
...
private:
    friend extern(C++) class QSGRootNode;
    friend extern(C++) class QSGBatchRenderer::Renderer;
...
protected:
    friend extern(C++) class QSGNodePrivate;

    QScopedPointer<QSGNodePrivate> d_ptr;
}

void Q_QUICK_EXPORT qsgnode_set_description(QSGNode *node, ref const(QString) description);

extern(C++) class Q_QUICK_EXPORT QSGBasicGeometryNode : QSGNode
{
public:
    ~QSGBasicGeometryNode();
...
private:
    friend extern(C++) class QSGNodeUpdater;
    friend extern(C++) class QSGBatchRenderer::Updater;
...
extern(C++) class QSGMaterial;

extern(C++) class Q_QUICK_EXPORT QSGGeometryNode : QSGBasicGeometryNode
{
public:
    QSGGeometryNode();
    ~QSGGeometryNode();
...
extern(C++) class Q_QUICK_EXPORT QSGClipNode : QSGBasicGeometryNode
{
public:
    QSGClipNode();
    ~QSGClipNode();
    ...
    void setClipRect(ref const(QRectF) );
...
extern(C++) class Q_QUICK_EXPORT QSGTransformNode : QSGNode
{
public:
    QSGTransformNode();
    ~QSGTransformNode();

    void setMatrix(ref const(QMatrix4x4) matrix);
    ref const(QMatrix4x4) matrix() const { return m_matrix; }

    void setCombinedMatrix(ref const(QMatrix4x4) matrix);
    ref const(QMatrix4x4) combinedMatrix() const { return m_combined_matrix; }
...
extern(C++) class Q_QUICK_EXPORT QSGRootNode : QSGNode
{
public:
    QSGRootNode();
    ~QSGRootNode();
...
private:
    ...
    friend extern(C++) class QSGAbstractRenderer;
    friend extern(C++) class QSGNode;
    friend extern(C++) class QSGGeometryNode;

    QList<QSGAbstractRenderer *> m_renderers;
}


extern(C++) class Q_QUICK_EXPORT QSGOpacityNode : QSGNode
{
public:
    QSGOpacityNode();
    ~QSGOpacityNode();
...
extern(C++) class Q_QUICK_EXPORT QSGNodeVisitor {
public:
