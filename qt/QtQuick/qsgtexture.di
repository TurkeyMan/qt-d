...
extern(C++) class QSGTexturePrivate;
extern(C++) class Q_QUICK_EXPORT QSGTexture : QObject
{
    mixin Q_OBJECT;
...
public:
    ...
    /+inline+/ QRectF convertToNormalizedSourceRect(ref const(QRectF) rect) const;

protected:
    QSGTexture(QSGTexturePrivate &dd);
}

QRectF QSGTexture::convertToNormalizedSourceRect(ref const(QRectF) rect) const
{
    QSize s = textureSize();
...
extern(C++) class Q_QUICK_EXPORT QSGDynamicTexture : QSGTexture
{
    mixin Q_OBJECT;
public:
