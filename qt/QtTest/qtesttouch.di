...
public import qt.QtTest.qtestassert;
public import qt.QtTest.qtestsystem;
public import qt.QtTest.qtestspontaneevent;
...
Q_GUI_EXPORT  void qt_handleTouchEvent(QWindow *w, QTouchDevice *device,
                                ref const(QList<QTouchEvent::TouchPoint>) points,
...
namespace QTest
{
    ...
    public:
        ...
        QTouchEventSequence& press(int touchId, ref const(QPoint) pt, QWindow *window = 0)
        {
        ...
        QTouchEventSequence& move(int touchId, ref const(QPoint) pt, QWindow *window = 0)
        {
            QTouchEvent::TouchPoint &p = point(touchId);
            p.setScreenPos(mapToScreen(window, pt));
        ...
        QTouchEventSequence& release(int touchId, ref const(QPoint) pt, QWindow *window = 0)
        {
            QTouchEvent::TouchPoint &p = point(touchId);
            p.setScreenPos(mapToScreen(window, pt));
...
#ifdef QT_WIDGETS_LIB
        QTouchEventSequence& press(int touchId, ref const(QPoint) pt, QWidget *widget = 0)
        {
            QTouchEvent::TouchPoint &p = point(touchId);
        ...
        QTouchEventSequence& move(int touchId, ref const(QPoint) pt, QWidget *widget = 0)
        {
            QTouchEvent::TouchPoint &p = point(touchId);
            p.setScreenPos(mapToScreen(widget, pt));
            p.setState(Qt.TouchPointMoved);
        ...
        QTouchEventSequence& release(int touchId, ref const(QPoint) pt, QWidget *widget = 0)
        {
            QTouchEvent::TouchPoint &p = point(touchId);
            p.setScreenPos(mapToScreen(widget, pt));
            p.setState(Qt.TouchPointReleased);
...
#ifdef QT_WIDGETS_LIB
        QPoint mapToScreen(QWidget *widget, ref const(QPoint) pt)
        {
            if (widget)
...
#endif
        QPoint mapToScreen(QWindow *window, ref const(QPoint) pt)
        {
            if(window)
