    ...
    do {\
        QT_TRY {\
            QT_TRY {\
                ...
                return;\
            } QT_CATCH (ref const(exceptiontype) ) {\
            }\
        } QT_CATCH (ref const(std::exception) e) {\
...
#define QTEST_COMPARE_DECL(KLASS)\
    template<> Q_TESTLIB_EXPORT char *toString<KLASS >(ref const(KLASS) );

namespace QTest
{
    template <typename T>
    /+inline+/ char *toString(ref const(T) )
    {
        return 0;
    }
    ...
    Q_TESTLIB_EXPORT int qExec(QObject *testObject, ref const(QStringList) arguments);
    ...
    Q_TESTLIB_EXPORT QString qFindTestData(ref const(QString) basepath, const(char)* file = 0, int line = 0, const(char)* builddir = 0);
    ...
    template <class T>
    /+inline+/ bool qTest(ref const(T) actual, const(char)* elementName, const(char)* actualStr,
                     const(char)* expected, const(char)* file, int line)
    {
