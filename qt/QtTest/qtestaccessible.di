...
public import qt.QtGui.qguiapplication;
...
bool operator==(ref const(QAccessibleEvent) l, ref const(QAccessibleEvent) r)
{
    if (l.type() != r.type()) {
...
//        qDebug() << "QAccessibleEvent for wrong object: " << l.object() << " and " << r.object() << " child: " << l.child() << " and " << r.child();
    ...
    if (l.type() == QAccessible::StateChanged) {
        return static_cast<ref const(QAccessibleStateChangeEvent*>()l)->changedStates()
                == static_cast<ref const(QAccessibleStateChangeEvent*>()r)->changedStates();
    } else if (l.type() == QAccessible::TextCaretMoved) {
        return static_cast<ref const(QAccessibleTextCursorEvent*>()l)->cursorPosition()
                == static_cast<ref const(QAccessibleTextCursorEvent*>()r)->cursorPosition();
    } else if (l.type() == QAccessible::TextSelectionChanged) {
        const(QAccessibleTextSelectionEvent)* le = static_cast<ref const(QAccessibleTextSelectionEvent*>()l);
        const(QAccessibleTextSelectionEvent)* re = static_cast<ref const(QAccessibleTextSelectionEvent*>()r);
        ...
        const(QAccessibleTextInsertEvent)* le = static_cast<ref const(QAccessibleTextInsertEvent*>()l);
        const(QAccessibleTextInsertEvent)* re = static_cast<ref const(QAccessibleTextInsertEvent*>()r);
        return  le->cursorPosition() == re->cursorPosition() &&
                le->changePosition() == re->changePosition() &&
                le->textInserted() == re->textInserted();
    } else if (l.type() == QAccessible::TextRemoved) {
        const(QAccessibleTextRemoveEvent)* le = static_cast<ref const(QAccessibleTextRemoveEvent*>()l);
        const(QAccessibleTextRemoveEvent)* re = static_cast<ref const(QAccessibleTextRemoveEvent*>()r);
        return  le->cursorPosition() == re->cursorPosition() &&
                le->changePosition() == re->changePosition() &&
                le->textRemoved() == re->textRemoved();
    } else if (l.type() == QAccessible::TextUpdated) {
        const(QAccessibleTextUpdateEvent)* le = static_cast<ref const(QAccessibleTextUpdateEvent*>()l);
        const(QAccessibleTextUpdateEvent)* re = static_cast<ref const(QAccessibleTextUpdateEvent*>()r);
        return  le->cursorPosition() == re->cursorPosition() &&
                le->changePosition() == re->changePosition() &&
                le->textInserted() == re->textInserted() &&
                le->textRemoved() == re->textRemoved();
    } else if (l.type() == QAccessible::ValueChanged) {
        const(QAccessibleValueChangeEvent)* le = static_cast<ref const(QAccessibleValueChangeEvent*>()l);
        const(QAccessibleValueChangeEvent)* re = static_cast<ref const(QAccessibleValueChangeEvent*>()r);
...
private:
    static QString msgAccessibilityEventListMismatch(ref const(EventList) haystack,
