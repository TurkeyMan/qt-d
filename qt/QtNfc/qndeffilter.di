...
public:
    QNdefFilter();
    QNdefFilter(ref const(QNdefFilter) other);
    ~QNdefFilter();
    ...
    void appendRecord(QNdefRecord::TypeNameFormat typeNameFormat, ref const(QByteArray) type,
                      unsigned int min = 1, unsigned int max = 1);
    void appendRecord(ref const(Record) record);

    int recordCount() const;
    Record recordAt(int i) const;

    QNdefFilter &operator=(ref const(QNdefFilter) other);
