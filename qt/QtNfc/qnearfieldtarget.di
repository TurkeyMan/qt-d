...
public:
    ...
    public:
        RequestId();
        RequestId(ref const(RequestId) other);
        ...
        bool operator<(ref const(RequestId) other) const;
        bool operator==(ref const(RequestId) other) const;
        bool operator!=(ref const(RequestId) other) const;
        RequestId &operator=(ref const(RequestId) other);
    ...
    /+virtual+/ RequestId writeNdefMessages(ref const(QList<QNdefMessage>) messages);

    // TagTypeSpecificAccess
    /+virtual+/ RequestId sendCommand(ref const(QByteArray) command);
    /+virtual+/ RequestId sendCommands(ref const(QList<QByteArray>) commands);

    /+virtual+/ bool waitForRequestCompleted(ref const(RequestId) id, int msecs = 5000);

    QVariant requestResponse(ref const(RequestId) id);
    void setResponseForRequest(ref const(QNearFieldTarget::RequestId) id, ref const(QVariant) response,
                               bool emitRequestCompleted = true);

protected:
    Q_INVOKABLE /+virtual+/ bool handleResponse(ref const(QNearFieldTarget::RequestId) id,
                                            ref const(QByteArray) response);

Q_SIGNALS:
    void disconnected();

    void ndefMessageRead(ref const(QNdefMessage) message);
    void ndefMessagesWritten();

    void requestCompleted(ref const(QNearFieldTarget::RequestId) id);

    void error(QNearFieldTarget::Error error, ref const(QNearFieldTarget::RequestId) id);
