...
public:
    ...
    QNdefRecord(ref const(QNdefRecord) other);
    QNdefRecord &operator=(ref const(QNdefRecord) other);

    void setTypeNameFormat(TypeNameFormat typeNameFormat);
    TypeNameFormat typeNameFormat() const;

    void setType(ref const(QByteArray) type);
    QByteArray type() const;

    void setId(ref const(QByteArray) id);
    QByteArray id() const;
    ...
    bool operator==(ref const(QNdefRecord) other) const;
    /+inline+/ bool operator!=(ref const(QNdefRecord) other) const { return !operator==(other); }

protected:
    QNdefRecord(ref const(QNdefRecord) other, TypeNameFormat typeNameFormat, ref const(QByteArray) type);
    QNdefRecord(ref const(QNdefRecord) other, TypeNameFormat typeNameFormat);
    QNdefRecord(TypeNameFormat typeNameFormat, ref const(QByteArray) type);
...
#define Q_DECLARE_NDEF_RECORD(className, typeNameFormat, type, initialPayload) \
    ...
    className(ref const(QNdefRecord) other) : QNdefRecord(other, typeNameFormat, type) { }
...
Q_NFC_EXPORT uint qHash(ref const(QNdefRecord) key);
