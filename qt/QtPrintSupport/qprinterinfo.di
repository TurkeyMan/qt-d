...
public import qt.QtPrintSupport.qprinter;
...
public:
    QPrinterInfo();
    QPrinterInfo(ref const(QPrinterInfo) other);
    explicit QPrinterInfo(ref const(QPrinter) printer);
    ~QPrinterInfo();

    QPrinterInfo &operator=(ref const(QPrinterInfo) other);
...
private:
    explicit QPrinterInfo(ref const(QString) name);
