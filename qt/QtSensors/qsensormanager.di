...
public:
    ...
    static void registerBackend(ref const(QByteArray) type, ref const(QByteArray) identifier, QSensorBackendFactory *factory);
    static void unregisterBackend(ref const(QByteArray) type, ref const(QByteArray) identifier);

    static bool isBackendRegistered(ref const(QByteArray) type, ref const(QByteArray) identifier);

    // Create a backend (uses the type and identifier set in the sensor)
    static QSensorBackend *createBackend(QSensor *sensor);

    static void setDefaultBackend(ref const(QByteArray) type, ref const(QByteArray) identifier);
};
