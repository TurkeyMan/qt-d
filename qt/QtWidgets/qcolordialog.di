...
class Q_WIDGETS_EXPORT QColorDialog : public QDialog
{
    ...
    mixin Q_PROPERTY!(ColorDialogOptions, "options", "READ", "options", "WRITE", "setOptions");
...
public:
    ...
    explicit QColorDialog(ref const(QColor) initial, QWidget *parent = 0);
    ~QColorDialog();

    void setCurrentColor(ref const(QColor) color);
    ...
    static QColor getColor(ref const(QColor) initial = Qt.white,
                           QWidget *parent = 0,
                           ref const(QString) title = QString(),
...
Q_SIGNALS:
    void currentColorChanged(ref const(QColor) color);
    void colorSelected(ref const(QColor) color);
