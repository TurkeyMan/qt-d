...
public import qt.QtWidgets.qscrollarea;
public import qt.QtWidgets.qgraphicsscene;
...
class Q_WIDGETS_EXPORT QGraphicsView : public QAbstractScrollArea
{
    ...
    mixin Q_PROPERTY!(DragMode, "dragMode", "READ", "dragMode", "WRITE", "setDragMode");
    mixin Q_PROPERTY!(CacheMode, "cacheMode", "READ", "cacheMode", "WRITE", "setCacheMode");
    mixin Q_PROPERTY!(ViewportAnchor, "transformationAnchor", "READ", "transformationAnchor", "WRITE", "setTransformationAnchor");
    mixin Q_PROPERTY!(ViewportAnchor, "resizeAnchor", "READ", "resizeAnchor", "WRITE", "setResizeAnchor");
    mixin Q_PROPERTY!(ViewportUpdateMode, "viewportUpdateMode", "READ", "viewportUpdateMode", "WRITE", "setViewportUpdateMode");
#ifndef QT_NO_RUBBERBAND
    mixin Q_PROPERTY!(Qt.ItemSelectionMode, "rubberBandSelectionMode", "READ", "rubberBandSelectionMode", "WRITE", "setRubberBandSelectionMode");
#endif
    mixin Q_PROPERTY!(OptimizationFlags, "optimizationFlags", "READ", "optimizationFlags", "WRITE", "setOptimizationFlags");
...
public:
    ...
    void centerOn(ref const(QPointF) pos);
    /+inline+/ void centerOn(qreal x, qreal y);
    void centerOn(const(QGraphicsItem)* item);
    void ensureVisible(ref const(QRectF) rect, int xmargin = 50, int ymargin = 50);
    /+inline+/ void ensureVisible(qreal x, qreal y, qreal w, qreal h, int xmargin = 50, int ymargin = 50);
    void ensureVisible(const(QGraphicsItem)* item, int xmargin = 50, int ymargin = 50);
    void fitInView(ref const(QRectF) rect, Qt.AspectRatioMode aspectRadioMode = Qt.IgnoreAspectRatio);
    ...
    void render(QPainter *painter, ref const(QRectF) target = QRectF(), ref const(QRect) source = QRect(),
                Qt.AspectRatioMode aspectRatioMode = Qt.KeepAspectRatio);

    QList<QGraphicsItem *> items() const;
    QList<QGraphicsItem *> items(ref const(QPoint) pos) const;
    /+inline+/ QList<QGraphicsItem *> items(int x, int y) const;
    QList<QGraphicsItem *> items(ref const(QRect) rect, Qt.ItemSelectionMode mode = Qt.IntersectsItemShape) const;
    /+inline+/ QList<QGraphicsItem *> items(int x, int y, int w, int h, Qt.ItemSelectionMode mode = Qt.IntersectsItemShape) const;
    QList<QGraphicsItem *> items(ref const(QPolygon) polygon, Qt.ItemSelectionMode mode = Qt.IntersectsItemShape) const;
    QList<QGraphicsItem *> items(ref const(QPainterPath) path, Qt.ItemSelectionMode mode = Qt.IntersectsItemShape) const;
    QGraphicsItem *itemAt(ref const(QPoint) pos) const;
    /+inline+/ QGraphicsItem *itemAt(int x, int y) const;

    QPointF mapToScene(ref const(QPoint) point) const;
    QPolygonF mapToScene(ref const(QRect) rect) const;
    QPolygonF mapToScene(ref const(QPolygon) polygon) const;
    QPainterPath mapToScene(ref const(QPainterPath) path) const;
    QPoint mapFromScene(ref const(QPointF) point) const;
    QPolygon mapFromScene(ref const(QRectF) rect) const;
    QPolygon mapFromScene(ref const(QPolygonF) polygon) const;
...
public Q_SLOTS:
    void updateScene(ref const(QList<QRectF>) rects);
    void invalidateScene(ref const(QRectF) rect = QRectF(), QGraphicsScene::SceneLayers layers = QGraphicsScene::AllLayers);
    void updateSceneRect(ref const(QRectF) rect);
...
#ifndef QT_NO_CURSOR
    Q_PRIVATE_SLOT(d_func(), void _q_setViewportCursor(ref const(QCursor) ))
