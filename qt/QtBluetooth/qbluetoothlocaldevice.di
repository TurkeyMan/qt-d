...
public import qt.QtBluetooth.QBluetoothHostInfo;
...
public:
    ...
    explicit QBluetoothLocalDevice(ref const(QBluetoothAddress) address, QObject *parent = 0);
    /+virtual+/ ~QBluetoothLocalDevice();

    bool isValid() const;

    void requestPairing(ref const(QBluetoothAddress) address, Pairing pairing);
    Pairing pairingStatus(ref const(QBluetoothAddress) address) const;
...
Q_SIGNALS:
    ...
    void deviceConnected(ref const(QBluetoothAddress) address);
    void deviceDisconnected(ref const(QBluetoothAddress) address);
    void pairingFinished(ref const(QBluetoothAddress) address, QBluetoothLocalDevice::Pairing pairing);

    void pairingDisplayPinCode(ref const(QBluetoothAddress) address, QString pin);
    void pairingDisplayConfirmation(ref const(QBluetoothAddress) address, QString pin);
