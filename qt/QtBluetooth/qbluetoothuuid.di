...
public:
    ...
    explicit QBluetoothUuid(ref const(QString) uuid);
    QBluetoothUuid(ref const(QBluetoothUuid) uuid);
    QBluetoothUuid(ref const(QUuid) uuid);
    ~QBluetoothUuid();

    bool operator==(ref const(QBluetoothUuid) other) const;
...
/+inline+/ QDebug operator<<(QDebug debug, ref const(QBluetoothUuid) uuid)
{
    debug << uuid.toString();
    return debug;
}
