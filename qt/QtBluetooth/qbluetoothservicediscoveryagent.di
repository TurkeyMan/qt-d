...
public import qt.QtBluetooth.QBluetoothServiceInfo;
public import qt.QtBluetooth.QBluetoothUuid;
public import qt.QtBluetooth.QBluetoothDeviceDiscoveryAgent;

#ifdef QT_BLUEZ_BLUETOOTH
public import qt.QtCore.qprocess;
#endif
...
public:
    ...
    explicit QBluetoothServiceDiscoveryAgent(ref const(QBluetoothAddress) deviceAdapter, QObject *parent = 0);
    ...
    void setUuidFilter(ref const(QList<QBluetoothUuid>) uuids);
    void setUuidFilter(ref const(QBluetoothUuid) uuid);
    QList<QBluetoothUuid> uuidFilter() const;
    bool setRemoteAddress(ref const(QBluetoothAddress) address);
...
Q_SIGNALS:
    void serviceDiscovered(ref const(QBluetoothServiceInfo) info);
    void finished();
    void canceled();
...
private:
    ...
    Q_PRIVATE_SLOT(d_func(), void _q_deviceDiscovered(ref const(QBluetoothDeviceInfo) info))
    ...
    Q_PRIVATE_SLOT(d_func(), void _q_processFetchedUuids(ref const(QBluetoothAddress) address,
                                                         ref const(QList<QBluetoothUuid>)))
