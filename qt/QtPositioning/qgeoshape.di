...
public:
    QGeoShape();
    QGeoShape(ref const(QGeoShape) other);
    ~QGeoShape();
    ...
    bool isValid() const;
    bool isEmpty() const;
    bool contains(ref const(QGeoCoordinate) coordinate) const;

    void extendShape(ref const(QGeoCoordinate) coordinate);

    bool operator==(ref const(QGeoShape) other) const;
    bool operator!=(ref const(QGeoShape) other) const;

    QGeoShape &operator=(ref const(QGeoShape) other);
...
Q_POSITIONING_EXPORT QDebug operator<<(QDebug, ref const(QGeoShape) );
#endif

#ifndef QT_NO_DATASTREAM
Q_POSITIONING_EXPORT QDataStream &operator<<(QDataStream &stream, ref const(QGeoShape) shape);
