...
public:
    QGeoLocation();
    QGeoLocation(ref const(QGeoLocation) other);

    ~QGeoLocation();

    QGeoLocation &operator=(ref const(QGeoLocation) other);

    bool operator==(ref const(QGeoLocation) other) const;
    bool operator!=(ref const(QGeoLocation) other) const {
        return !(other == *this);
    }

    QGeoAddress address() const;
    void setAddress(ref const(QGeoAddress) address);
    QGeoCoordinate coordinate() const;
    void setCoordinate(ref const(QGeoCoordinate) position);
    QGeoRectangle boundingBox() const;
    void setBoundingBox(ref const(QGeoRectangle) box);
