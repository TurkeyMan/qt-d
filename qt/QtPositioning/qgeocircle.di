...
public:
    QGeoCircle();
    QGeoCircle(ref const(QGeoCoordinate) center, qreal radius = -1.0);
    QGeoCircle(ref const(QGeoCircle) other);
    QGeoCircle(ref const(QGeoShape) other);

    ~QGeoCircle();

    QGeoCircle &operator=(ref const(QGeoCircle) other);

    using QGeoShape::operator==;
    bool operator==(ref const(QGeoCircle) other) const;
