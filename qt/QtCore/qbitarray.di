...
extern(C++) class QBitRef;
extern(C++) class export QBitArray
{
    friend export QDataStream &operator<<(QDataStream &, ref const(QBitArray) );
    friend export QDataStream &operator>>(QDataStream &, QBitArray &);
    friend export uint qHash(ref const(QBitArray) key, uint seed) nothrow;
    QByteArray d;

public:
    /+inline+/ QBitArray() {}
    explicit QBitArray(int size, bool val = false);
    QBitArray(ref const(QBitArray) other) : d(other.d) {}
    /+inline+/ QBitArray &operator=(ref const(QBitArray) other) { d = other.d; return *this; }
    ...
    QBitArray& operator&=(ref const(QBitArray) );
    QBitArray& operator|=(ref const(QBitArray) );
    QBitArray& operator^=(ref const(QBitArray) );
    QBitArray  operator~() const;

    /+inline+/ bool operator==(ref const(QBitArray) other) const { return d == other.d; }
    /+inline+/ bool operator!=(ref const(QBitArray) other) const { return d != other.d; }
...
export QBitArray operator&(ref const(QBitArray) , ref const(QBitArray) );
export QBitArray operator|(ref const(QBitArray) , ref const(QBitArray) );
export QBitArray operator^(ref const(QBitArray) , ref const(QBitArray) );

/+inline+/ bool QBitArray::testBit(int i) const
{ Q_ASSERT(uint(i) < uint(size()));
 return (*(reinterpret_cast<ref const(uchar*>(d.constData())+1+(i>>3)))  (1 << (i & 7))) != 0; }
...
export QDataStream &operator<<(QDataStream &, ref const(QBitArray) );
export QDataStream &operator>>(QDataStream &, QBitArray &);
#endif

#ifndef QT_NO_DEBUG_STREAM
export QDebug operator<<(QDebug, ref const(QBitArray) );
#endif
