...
template <typename T> struct QAtomicOps: QGenericAtomicOps<QAtomicOps<T> >
{
    ...
    static bool ref(T &_q_value) nothrow
    {
        return ++_q_value != 0;
    }
    static bool deref(T &_q_value) nothrow
    {
        return --_q_value != 0;
    }

    static bool testAndSetRelaxed(T &_q_value, T expectedValue, T newValue, T *currentValue = 0) nothrow
    {
        if (currentValue)
    ...
    T fetchAndAddRelaxed(T &_q_value, AdditiveType valueToAdd) nothrow
    {
