...
template <class Key> /+inline+/ bool qMapLessThanKey(ref const(Key) key1, ref const(Key) key2)
{
    return key1 < key2;
}
...
struct QMapDataBase;
template <class Key, extern(C++) class T> struct QMapData;

struct export QMapNodeBase
{
    quintptr p;
    QMapNodeBase *left;
    QMapNodeBase *right;
...
struct QMapNode : public QMapNodeBase
{
    ...
    QMapNode<Key, T> *lowerBound(ref const(Key) key);
    QMapNode<Key, T> *upperBound(ref const(Key) key);
...
template <class Key, extern(C++) class T>
/+inline+/ QMapNode<Key, T> *QMapNode<Key, T>::lowerBound(ref const(Key) akey)
{
...
template <class Key, extern(C++) class T>
/+inline+/ QMapNode<Key, T> *QMapNode<Key, T>::upperBound(ref const(Key) akey)
{
    QMapNode<Key, T> *n = this;
    QMapNode<Key, T> *lastNode = 0;
    while (n) {
...
struct QMapData : public QMapDataBase
{
    ...
    Node *findNode(ref const(Key) akey) const;
    void nodeRange(ref const(Key) akey, Node **firstNode, Node **lastNode);

    Node *createNode(ref const(Key) k, ref const(T) v, Node *parent = 0, bool left = false)
    {
...
template <class Key, extern(C++) class T>
QMapNode<Key, T> *QMapData<Key, T>::findNode(ref const(Key) akey) const
{
    if (Node *r = root()) {
...
template <class Key, extern(C++) class T>
void QMapData<Key, T>::nodeRange(ref const(Key) akey, QMapNode<Key, T> **firstNode, QMapNode<Key, T> **lastNode)
{
    Node *n = root();
    Node *l = end();
    while (n) {
...
public:
    ...
    const Key key(ref const(T) value, ref const(Key) defaultKey = Key()) const;
    const T value(ref const(Key) key, ref const(T) defaultValue = T()) const;
    ...
    /+inline+/ ref const(Key) firstKey() const { Q_ASSERT(!isEmpty()); return constBegin().key(); }
    /+inline+/ ref const(Key) lastKey() const { Q_ASSERT(!isEmpty()); return (constEnd() - 1).key(); }

    /+inline+/ T &first() { Q_ASSERT(!isEmpty()); return *begin(); }
    /+inline+/ ref const(T) first() const { Q_ASSERT(!isEmpty()); return *constBegin(); }
    /+inline+/ T &last() { Q_ASSERT(!isEmpty()); return *(end() - 1); }
    /+inline+/ ref const(T) last() const { Q_ASSERT(!isEmpty()); return *(constEnd() - 1); }
    ...
    public:
        ...
        /+inline+/ ref const(Key) key() const { return i->key; }
    ...
    public:
        ...
        /+inline+/ bool operator!=(ref const(const_iterator) o) const
            { return i != o.i; }
#endif
        friend extern(C++) class QMap<Key, T>;
    }
    ...
    public:
        ...
        /+inline+/ ref const(Key) key() const { return i->key; }
        /+inline+/ ref const(T) value() const { return i->value; }
        /+inline+/ ref const(T) operator*() const { return i->value; }
    ...
    iterator lowerBound(ref const(Key) key);
    const_iterator lowerBound(ref const(Key) key) const;
    iterator upperBound(ref const(Key) key);
    const_iterator upperBound(ref const(Key) key) const;
    iterator insert(ref const(Key) key, ref const(T) value);
    iterator insert(const_iterator pos, ref const(Key) key, ref const(T) value);
    iterator insertMulti(ref const(Key) key, ref const(T) value);
    iterator insertMulti(const_iterator pos, ref const(Key) akey, ref const(T) avalue);
    ...
    QPair<iterator, iterator> equal_range(ref const(Key) akey);
...
private:
    void detach_helper();
    bool isValidIterator(ref const(const_iterator) ci) const
    {
...
template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE const T QMap<Key, T>::value(ref const(Key) akey, ref const(T) adefaultValue) const
{
...
template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE const T QMap<Key, T>::operator[](ref const(Key) akey) const
{
    return value(akey);
}

template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE T &QMap<Key, T>::operator[](ref const(Key) akey)
{
    detach();
    Node *n = d->findNode(akey);
    if (!n)
...
template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE int QMap<Key, T>::count(ref const(Key) akey) const
{
    Node *firstNode;
    Node *lastNode;
...
template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE bool QMap<Key, T>::contains(ref const(Key) akey) const
{
    return d->findNode(akey) != 0;
}

template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE typename QMap<Key, T>::iterator QMap<Key, T>::insert(ref const(Key) akey, ref const(T) avalue)
{
    detach();
    Node *n = d->root();
    Node *y = d->end();
...
template <class Key, extern(C++) class T>
typename QMap<Key, T>::iterator QMap<Key, T>::insert(const_iterator pos, ref const(Key) akey, ref const(T) avalue)
{
    if (d->ref.isShared())
...
template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE typename QMap<Key, T>::iterator QMap<Key, T>::insertMulti(ref const(Key) akey,
                                                                            ref const(T) avalue)
{
    detach();
    Node* y = d->end();
...
template <class Key, extern(C++) class T>
typename QMap<Key, T>::iterator QMap<Key, T>::insertMulti(const_iterator pos, ref const(Key) akey, ref const(T) avalue)
{
    if (d->ref.isShared())
...
template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE typename QMap<Key, T>::const_iterator QMap<Key, T>::constFind(ref const(Key) akey) const
{
    Node *n = d->findNode(akey);
...
template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE typename QMap<Key, T>::const_iterator QMap<Key, T>::find(ref const(Key) akey) const
{
    return constFind(akey);
}

template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE typename QMap<Key, T>::iterator QMap<Key, T>::find(ref const(Key) akey)
{
    detach();
    Node *n = d->findNode(akey);
...
template <class Key, extern(C++) class T>
QPair<typename QMap<Key, T>::iterator, typename QMap<Key, T>::iterator> QMap<Key, T>::equal_range(ref const(Key) akey)
{
    detach();
...
template <class Key, extern(C++) class T>
Q_OUTOFLINE_TEMPLATE int QMap<Key, T>::remove(ref const(Key) akey)
{
    detach();
    int n = 0;
...
template <class Key, extern(C++) class T>
Q_OUTOFLINE_TEMPLATE T QMap<Key, T>::take(ref const(Key) akey)
{
    detach();
...
template <class Key, extern(C++) class T>
Q_OUTOFLINE_TEMPLATE QList<Key> QMap<Key, T>::keys(ref const(T) avalue) const
{
    QList<Key> res;
    const_iterator i = begin();
    while (i != end()) {
...
template <class Key, extern(C++) class T>
Q_OUTOFLINE_TEMPLATE const Key QMap<Key, T>::key(ref const(T) avalue, ref const(Key) defaultKey) const
{
    const_iterator i = begin();
    while (i != end()) {
        if (i.value() == avalue)
...
template <class Key, extern(C++) class T>
Q_OUTOFLINE_TEMPLATE QList<T> QMap<Key, T>::values(ref const(Key) akey) const
{
    QList<T> res;
    Node *n = d->findNode(akey);
    if (n) {
...
template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE typename QMap<Key, T>::const_iterator QMap<Key, T>::lowerBound(ref const(Key) akey) const
{
...
template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE typename QMap<Key, T>::iterator QMap<Key, T>::lowerBound(ref const(Key) akey)
{
    detach();
    Node *lb = d->root() ? d->root()->lowerBound(akey) : 0;
    if (!lb)
...
QMap<Key, T>::upperBound(ref const(Key) akey) const
{
...
template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE typename QMap<Key, T>::iterator QMap<Key, T>::upperBound(ref const(Key) akey)
{
    detach();
    Node *ub = d->root() ? d->root()->upperBound(akey) : 0;
    if (!ub)
...
template <class Key, extern(C++) class T>
extern(C++) class QMultiMap : QMap<Key, T>
{
public:
    QMultiMap() {}
#ifdef Q_COMPILER_INITIALIZER_LISTS
    ...
    /+inline+/ typename QMap<Key, T>::iterator replace(ref const(Key) key, ref const(T) value)
    { return QMap<Key, T>::insert(key, value); }
    /+inline+/ typename QMap<Key, T>::iterator insert(ref const(Key) key, ref const(T) value)
    { return QMap<Key, T>::insertMulti(key, value); }
    /+inline+/ typename QMap<Key, T>::iterator insert(typename QMap<Key, T>::const_iterator pos, ref const(Key) key, ref const(T) value)
    { return QMap<Key, T>::insertMulti(pos, key, value); }

    /+inline+/ QMultiMap &operator+=(ref const(QMultiMap) other)
    { this->unite(other); return *this; }
    /+inline+/ QMultiMap operator+(ref const(QMultiMap) other) const
    ...
    /+inline+/ typename QMap<Key, T>::iterator find(ref const(Key) key)
    { return QMap<Key, T>::find(key); }
    /+inline+/ typename QMap<Key, T>::const_iterator find(ref const(Key) key) const
    { return QMap<Key, T>::find(key); }
    /+inline+/ typename QMap<Key, T>::const_iterator constFind(ref const(Key) key) const
    ...
    typename QMap<Key, T>::iterator find(ref const(Key) key, ref const(T) value) {
    ...
    typename QMap<Key, T>::const_iterator find(ref const(Key) key, ref const(T) value) const {
    ...
    typename QMap<Key, T>::const_iterator constFind(ref const(Key) key, ref const(T) value) const
...
template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE bool QMultiMap<Key, T>::contains(ref const(Key) key, ref const(T) value) const
{
    return constFind(key, value) != QMap<Key, T>::constEnd();
}

template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE int QMultiMap<Key, T>::remove(ref const(Key) key, ref const(T) value)
{
    int n = 0;
    typename QMap<Key, T>::iterator i(find(key));
...
template <class Key, extern(C++) class T>
Q_INLINE_TEMPLATE int QMultiMap<Key, T>::count(ref const(Key) key, ref const(T) value) const
{
    int n = 0;
    typename QMap<Key, T>::const_iterator i(constFind(key));
    typename QMap<Key, T>::const_iterator end(QMap<Key, T>::constEnd());
