...
extern(C++) class QAbstractFileEngine;
extern(C++) class QSaveFilePrivate;

extern(C++) class export QSaveFile : QFileDevice
{
    mixin Q_OBJECT;
...
public:
    ...
    explicit QSaveFile(ref const(QString) name);
    explicit QSaveFile(QObject *parent = 0);
    explicit QSaveFile(ref const(QString) name, QObject *parent);
    ~QSaveFile();
