...
template <typename T> extern(C++) class QSet;

template <typename T> struct QListSpecialMethods { }
template <> struct QListSpecialMethods<QByteArray>;

struct export QListData {
    struct Data {
...
template <typename T>
extern(C++) class QList : QListSpecialMethods<T>
{
    struct Node { void *v;
#if defined(Q_CC_BOR)
...
public:
    ...
    QList(ref const(QList<T>) l);
    ~QList();
    QList<T> &operator=(ref const(QList<T>) l);
    ...
    /+inline+/ bool operator!=(ref const(QList<T>) l) const { return !(*this == l); }
...
#endif
    /+inline+/ bool isSharedWith(ref const(QList<T>) other) const { return d == other.d; }
    ...
    void append(ref const(QList<T>) t);
    void prepend(ref const(T) t);
    void insert(int i, ref const(T) t);
    void replace(int i, ref const(T) t);
    void removeAt(int i);
    ...
    public:
        ...
        /+inline+/ iterator(ref const(iterator) o): i(o.i){}
    ...
    public:
        ...
        /+inline+/ const_iterator(ref const(const_iterator) o): i(o.i) {}
#ifdef QT_STRICT_ITERATORS
        /+inline+/ explicit const_iterator(ref const(iterator) o): i(o.i) {}
#else
        /+inline+/ const_iterator(ref const(iterator) o): i(o.i) {}
#endif
        /+inline+/ ref const(T) operator*() const { return i->t(); }
        /+inline+/ const(T)* operator->() const { return &i->t(); }
        /+inline+/ ref const(T) operator[](difference_type j) const { return i[j].t(); }
    ...
    iterator insert(iterator before, ref const(T) t);
    ...
    /+inline+/ ref const(T) first() const { Q_ASSERT(!isEmpty()); return at(0); }
    T& last() { Q_ASSERT(!isEmpty()); return *(--end()); }
    ref const(T) last() const { Q_ASSERT(!isEmpty()); return at(count() - 1); }
    /+inline+/ void removeFirst() { Q_ASSERT(!isEmpty()); erase(begin()); }
    /+inline+/ void removeLast() { Q_ASSERT(!isEmpty()); erase(--end()); }
    /+inline+/ bool startsWith(ref const(T) t) const { return !isEmpty() && first() == t; }
    /+inline+/ bool endsWith(ref const(T) t) const { return !isEmpty() && last() == t; }
    ...
    // stl compatibility
    /+inline+/ void push_back(ref const(T) t) { append(t); }
    /+inline+/ void push_front(ref const(T) t) { prepend(t); }
    /+inline+/ T& front() { return first(); }
    /+inline+/ ref const(T) front() const { return first(); }
    /+inline+/ T& back() { return last(); }
    /+inline+/ ref const(T) back() const { return last(); }
    ...
    // comfort
    QList<T> &operator+=(ref const(QList<T>) l);
    /+inline+/ QList<T> operator+(ref const(QList<T>) l) const
    { QList n = *this; n += l; return n; }
    /+inline+/ QList<T> &operator+=(ref const(T) t)
    { append(t); return *this; }
    /+inline+/ QList<T> &operator<< (ref const(T) t)
    { append(t); return *this; }
    /+inline+/ QList<T> &operator<<(ref const(QList<T>) l)
    { *this += l; return *this; }
    ...
    static QList<T> fromSet(ref const(QSet<T>) set);

    static /+inline+/ QList<T> fromStdList(ref const(std::list<T>) list)
...
private:
    ...
    void node_construct(Node *n, ref const(T) t);
...
template <typename T>
Q_INLINE_TEMPLATE void QList<T>::node_construct(Node *n, ref const(T) t)
{
...
template <typename T>
Q_INLINE_TEMPLATE QList<T> &QList<T>::operator=(ref const(QList<T>) l)
{
    if (d != l.d) {
        QList<T> tmp(l);
        tmp.swap(*this);
...
template <typename T>
/+inline+/ typename QList<T>::iterator QList<T>::insert(iterator before, ref const(T) t)
{
...
template <typename T>
/+inline+/ ref const(T) QList<T>::at(int i) const
{ Q_ASSERT_X(i >= 0 && i < p.size(), "QList<T>::at", "index out of range");
 return reinterpret_cast<Node *>(p.at(i))->t(); }
template <typename T>
/+inline+/ ref const(T) QList<T>::operator[](int i) const
...
template <typename T>
Q_OUTOFLINE_TEMPLATE void QList<T>::append(ref const(T) t)
{
    if (d->ref.isShared()) {
...
template <typename T>
/+inline+/ void QList<T>::prepend(ref const(T) t)
{
    if (d->ref.isShared()) {
...
template <typename T>
/+inline+/ void QList<T>::insert(int i, ref const(T) t)
{
    if (d->ref.isShared()) {
...
template <typename T>
/+inline+/ void QList<T>::replace(int i, ref const(T) t)
{
...
template<typename T>
Q_OUTOFLINE_TEMPLATE T QList<T>::value(int i, ref const(T) defaultValue) const
{
...
template <typename T>
Q_OUTOFLINE_TEMPLATE QList<T>::QList(ref const(QList<T>) l)
...
template <typename T>
Q_OUTOFLINE_TEMPLATE bool QList<T>::operator==(ref const(QList<T>) l) const
{
    if (d == l.d)
        return true;
    if (p.size() != l.p.size())
...
template <typename T>
Q_OUTOFLINE_TEMPLATE int QList<T>::removeAll(ref const(T) _t)
{
...
template <typename T>
Q_OUTOFLINE_TEMPLATE bool QList<T>::removeOne(ref const(T) _t)
{
    int index = indexOf(_t);
    if (index != -1) {
        removeAt(index);
...
template <typename T>
Q_OUTOFLINE_TEMPLATE QList<T> &QList<T>::operator+=(ref const(QList<T>) l)
{
    if (!l.isEmpty()) {
        if (isEmpty()) {
            *this = l;
...
template <typename T>
/+inline+/ void QList<T>::append(ref const(QList<T>) t)
{
    *this += t;
}

template <typename T>
Q_OUTOFLINE_TEMPLATE int QList<T>::indexOf(ref const(T) t, int from) const
{
    if (from < 0)
...
template <typename T>
Q_OUTOFLINE_TEMPLATE int QList<T>::lastIndexOf(ref const(T) t, int from) const
{
    if (from < 0)
        from += p.size();
    else if (from >= p.size())
...
template <typename T>
Q_OUTOFLINE_TEMPLATE bool QList<T>::contains(ref const(T) t) const
{
    Node *b = reinterpret_cast<Node *>(p.begin());
    Node *i = reinterpret_cast<Node *>(p.end());
    while (i-- != b)
...
template <typename T>
Q_OUTOFLINE_TEMPLATE int QList<T>::count(ref const(T) t) const
{
    int c = 0;
    Node *b = reinterpret_cast<Node *>(p.begin());
    Node *i = reinterpret_cast<Node *>(p.end());
