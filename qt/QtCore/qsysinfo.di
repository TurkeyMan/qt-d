...
export extern(C++) class QSysInfo {
public:
    enum Sizes {
...
} else static if(Q_BYTE_ORDER) {
    enum Endian {
        BigEndian,
        LittleEndian,
        ByteOrder = Q_BYTE_ORDER == Q_BIG_ENDIAN ? BigEndian : LittleEndian
    }
} else {
    static assert(false, "Undefined byte order");
}

version(Q_OS_WIN) version = Q_OS_WIN_OR_CYGWIN;
version(Q_OS_CYGWIN) version = Q_OS_WIN_OR_CYGWIN;
version(Q_OS_WIN_OR_CYGWIN) {
    enum WinVersion {
        WV_32s      = 0x0001,
        WV_95       = 0x0002,
        WV_98       = 0x0003,
...
version(Q_OS_MAC) {
    auto Q_MV_IOS(major, minor) { return QSysInfo.MV_IOS | major << 4 | minor; }
    enum MacVersion {
        MV_Unknown = 0x0000,
