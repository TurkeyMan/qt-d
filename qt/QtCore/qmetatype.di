...
public import QtCore.qvarlengtharray;
public import QtCore.qisenum;
public import QtCore.qtypetraits;
...
extern(C++) class QMetaTypeInterface;
struct QMetaObject;
...
To convertImplicit(ref const(From) from)
{
    return from;
}
...
extern(C++) class export QMetaType {
...
public:
    ...
    static int registerNormalizedType(ref const(QT_PREPEND_NAMESPACE(QByteArray)) normalizedTypeName, Deleter deleter,
                            Creator creator,
                            Destructor destructor,
                            Constructor constructor,
                            int size,
    ...
    static int registerNormalizedTypedef(ref const(QT_PREPEND_NAMESPACE(QByteArray)) normalizedTypeName, int aliasId);
...
private:
    ...
    /+inline+/ QMetaType(const ExtensionFlag extensionFlags, const(QMetaTypeInterface)* info,
                     ...
                     int typeId,
                     const(QMetaObject)* metaObject);
    QMetaType(ref const(QMetaType) other);
    QMetaType &operator =(ref const(QMetaType) );
...
struct VariantData
{
    ...
    VariantData(ref const(VariantData) other)
...
private:
    ...
    VariantData &operator=(ref const(VariantData) ) Q_DECL_EQ_DELETE;
}
...
struct export VectorBoolElements
{
...
public:
    ...
    static const(void)* getData(ref const(std::vector<bool>::const_iterator) it)
    {
...
extern(C++) class QSequentialIterableImpl
{
public:
...
public:
    ...
    /+inline+/ bool equal(ref const(QSequentialIterableImpl)other) const { return _equalIter(&_iterator, &other._iterator); }
    ...
    void copy(ref const(QSequentialIterableImpl) other)
    {
      *this = other;
...
{
    QSequentialIterableImpl operator()(ref const(From) f) const
    {
...
extern(C++) class QAssociativeIterableImpl
{
public:
    const(void)* _iterable;
    void *_iterator;
...
public:
    ...
    /+inline+/ bool equal(ref const(QAssociativeIterableImpl)other) const { return _equalIter(&_iterator, &other._iterator); }
    ...
    /+inline+/ void find(ref const(VariantData) key)
    { _find(_iterable, key.data, &_iterator); }

    int size() const { Q_ASSERT(_iterable); return _size(_iterable); }

    void copy(ref const(QAssociativeIterableImpl) other)
    {
      *this = other;
      _copyIter(&_iterator, &other._iterator);
    }
...
{
    QAssociativeIterableImpl operator()(ref const(From) f) const
    {
        return QAssociativeIterableImpl(&f);
    }
}

extern(C++) class QPairVariantInterfaceImpl
{
    const(void)* _pair;
    ...
    typedef VariantData (*getFunc)(const(void*)* p, int metaTypeId, uint flags);

    getFunc _getFirst;
    getFunc _getSecond;

    template<class T>
    static VariantData getFirstImpl(const(void*)* pair, int metaTypeId, uint flags)
    { return VariantData(metaTypeId, &static_cast<const T*>(*pair)->first, flags); }
    template<class T>
    static VariantData getSecondImpl(const(void*)* pair, int metaTypeId, uint flags)
...
#define QT_FORWARD_DECLARE_SHARED_POINTER_TYPES_ITER(Name) \
    template <class T> extern(C++) class Name; \
...
namespace QtPrivate
{
    ...
    export bool isBuiltinType(ref const(QByteArray) type);
} // namespace QtPrivate
...
template <typename T>
struct QMetaTypeId2<ref const(T)> : QMetaTypeId2<T> {}
...
namespace QtPrivate {
    ...
    template <class Result, extern(C++) class Arg0> struct IsPointerToTypeDerivedFromQObject<Result(*)(Arg0)> { enum { Value = false } }
    template <class Result, extern(C++) class Arg0, extern(C++) class Arg1> struct IsPointerToTypeDerivedFromQObject<Result(*)(Arg0, Arg1)> { enum { Value = false } }
    template <class Result, extern(C++) class Arg0, extern(C++) class Arg1, extern(C++) class Arg2> struct IsPointerToTypeDerivedFromQObject<Result(*)(Arg0, Arg1, Arg2)> { enum { Value = false } }
#else
    ...
    {
        QObject* operator()(ref const(QWeakPointer<T>) p) const
        {
            return p.data();
        }
    }
...
template <typename T>
int qRegisterNormalizedMetaType(ref const(QT_PREPEND_NAMESPACE(QByteArray)) normalizedTypeName
#ifndef Q_QDOC
    , T * dummy = 0
...
template <typename T>
QT_DEPRECATED /+inline+/ int qMetaTypeId(T *)
{ return qMetaTypeId<T>(); }
#ifndef Q_CC_SUN
template <typename T>
QT_DEPRECATED /+inline+/ int qRegisterMetaType(T *)
...
#define Q_DECLARE_BUILTIN_METATYPE(TYPE, METATYPEID, NAME) \
    ...
    { \
        ...
        static /+inline+/ int qt_metatype_id() { return METATYPEID; } \
    } \
    QT_END_NAMESPACE
...
#define Q_DECLARE_SEQUENTIAL_CONTAINER_METATYPE_ITER(TEMPLATENAME) \
    QT_BEGIN_NAMESPACE \
    template <class T> extern(C++) class TEMPLATENAME; \
    QT_END_NAMESPACE \
...
#define Q_FORWARD_DECLARE_METATYPE_TEMPLATE_2ARG_ITER(TEMPLATENAME, CPPTYPE) \
    QT_BEGIN_NAMESPACE \
    template <class T1, extern(C++) class T2> CPPTYPE TEMPLATENAME; \
    QT_END_NAMESPACE \
...
namespace QtMetaTypePrivate {
/+inline+/ bool isBuiltinSequentialType(int typeId)
{
...
/+inline+/ bool isBuiltinAssociativeType(int typeId)
{
