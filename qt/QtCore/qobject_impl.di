...
namespace QtPrivate {
    ...
    */
    ...
    // internal base extern(C++) class (interface) containing functions required to call a slot managed by a pointer to function.
    extern(C++) class QSlotObjectBase {
        QAtomicInt m_ref;
        // don't use /+virtual+/ functions here; we don't want the
        // compiler to create tons of per-polymorphic-extern(C++) class stuff that
    ...
    public:
        ...
        /+inline+/ int ref() nothrow { return m_ref.ref(); }
        /+inline+/ void destroyIfLastRef() nothrow
    ...
    template<typename Func, typename Args, typename R> extern(C++) class QSlotObject : QSlotObjectBase
    {
    ...
    // Args and R are the List of arguments and the returntype of the signal to which the slot is connected.
    template<typename Func, typename Args, typename R> extern(C++) class QStaticSlotObject : QSlotObjectBase
    {
        typedef QtPrivate::FunctionPointer<Func> FuncType;
        Func function;
        static void impl(int which, QSlotObjectBase *this_, QObject *r, void **a, bool *ret)
    ...
    // Args and R are the List of arguments and the returntype of the signal to which the slot is connected.
    template<typename Func, int N, typename Args, typename R> extern(C++) class QFunctorSlotObject : QSlotObjectBase
    {
    ...
    public:
        explicit QFunctorSlotObject(ref const(Func) f) : QSlotObjectBase(&impl), function(f) {}
    }
}
