...
extern(C++) class QCache
{
    ...
    /+inline+/ T *relink(ref const(Key) key) {
...
public:
    ...
    bool insert(ref const(Key) key, T *object, int cost = 1);
    T *object(ref const(Key) key) const;
    /+inline+/ bool contains(ref const(Key) key) const { return hash.contains(key); }
    T *operator[](ref const(Key) key) const;

    bool remove(ref const(Key) key);
...
template <class Key, extern(C++) class T>
/+inline+/ T *QCache<Key,T>::object(ref const(Key) key) const
{ return const_cast<QCache<Key,T>*>(this)->relink(key); }

template <class Key, extern(C++) class T>
/+inline+/ T *QCache<Key,T>::operator[](ref const(Key) key) const
{ return object(key); }

template <class Key, extern(C++) class T>
/+inline+/ bool QCache<Key,T>::remove(ref const(Key) key)
{
    typename QHash<Key, Node>::iterator i = hash.find(key);
    if (typename QHash<Key, Node>::const_iterator(i) == hash.constEnd()) {
        return false;
...
template <class Key, extern(C++) class T>
/+inline+/ T *QCache<Key,T>::take(ref const(Key) key)
{
    typename QHash<Key, Node>::iterator i = hash.find(key);
    if (i == hash.end())
        return 0;
...
template <class Key, extern(C++) class T>
bool QCache<Key,T>::insert(ref const(Key) akey, T *aobject, int acost)
{
    remove(akey);
    if (acost > mx) {
        delete aobject;
