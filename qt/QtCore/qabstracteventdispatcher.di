...
extern(C++) class QAbstractEventDispatcherPrivate;
extern(C++) class QSocketNotifier;

#ifdef Q_OS_WIN
extern(C++) class QWinEventNotifier;
#endif

extern(C++) class export QAbstractEventDispatcher : QObject
{
    mixin Q_OBJECT;
...
public:
    ...
    bool filterNativeEvent(ref const(QByteArray) eventType, void *message, long *result);
#if QT_DEPRECATED_SINCE(5, 0)
