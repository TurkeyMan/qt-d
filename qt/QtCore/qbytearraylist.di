...
namespace QtPrivate {
    QByteArray export QByteArrayList_join(const(QByteArrayList)* that, const(char)* separator, int separatorLength);
}

#ifdef Q_QDOC
extern(C++) class QByteArrayList : QList<QByteArray>
#else
...
public:
    ...
    /+inline+/ QByteArray join(ref const(QByteArray) sep) const
