...
namespace QtPrivate {
    ...
    template <typename T> struct RemoveConstRef<ref const(T)> { typedef T Type; }
    ...
    */
    ...
    void operator,(ref const(T) value, ref const(ApplyReturnValue<U>) container) {
        if (container.data)
            *reinterpret_cast<U*>(container.data) = value;
    }
#ifdef Q_COMPILER_RVALUE_REFS
    template<typename T, typename U>
    void operator,(T &&value, ref const(ApplyReturnValue<U>) container) {
        if (container.data)
            *reinterpret_cast<U*>(container.data) = value;
    }
#endif
    template<typename T>
    void operator,(T, ref const(ApplyReturnValue<void>) ) {}
    ...
    template <int... II, typename... SignalArgs, typename R, typename... SlotArgs, typename SlotRet, extern(C++) class Obj>
