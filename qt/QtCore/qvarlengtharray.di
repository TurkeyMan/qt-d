...
public import QtCore.qcontainerfwd;
...
extern(C++) class QPodList;

// Prealloc = 256 by default, specified in qcontainerfwd.h
template<class T, int Prealloc>
extern(C++) class QVarLengthArray
{
public:
    ...
    ref const(T) last() const { Q_ASSERT(!isEmpty()); return *(end() - 1); }
    ...
    /+inline+/ int indexOf(ref const(T) t, int from = 0) const;
    /+inline+/ int lastIndexOf(ref const(T) t, int from = -1) const;
    /+inline+/ bool contains(ref const(T) t) const;

    /+inline+/ T &operator[](int idx) {
        Q_ASSERT(idx >= 0 && idx < s);
        return ptr[idx];
    }
    /+inline+/ ref const(T) operator[](int idx) const {
        Q_ASSERT(idx >= 0 && idx < s);
        return ptr[idx];
    }
    /+inline+/ ref const(T) at(int idx) const { return operator[](idx); }

    T value(int i) const;
    T value(int i, ref const(T) defaultValue) const;

    /+inline+/ void append(ref const(T) t) {
        if (s == a)   // i.e. s != 0
            realloc(s, s<<1);
        const int idx = s++;
        if (QTypeInfo<T>::isComplex) {
    ...
    /+inline+/ QVarLengthArray<T, Prealloc> &operator<<(ref const(T) t)
    ...
    iterator insert(const_iterator before, int n, ref const(T) x);
    /+inline+/ iterator insert(const_iterator before, ref const(T) x) { return insert(before, 1, x); }
...
private:
    friend extern(C++) class QPodList<T, Prealloc>;
    ...
    bool isValidIterator(ref const(const_iterator) i) const
    {
...
template <class T, int Prealloc>
Q_INLINE_TEMPLATE int QVarLengthArray<T, Prealloc>::indexOf(ref const(T) t, int from) const
{
    if (from < 0)
        from = qMax(from + s, 0);
    if (from < s) {
...
template <class T, int Prealloc>
Q_INLINE_TEMPLATE int QVarLengthArray<T, Prealloc>::lastIndexOf(ref const(T) t, int from) const
{
    if (from < 0)
        from += s;
    else if (from >= s)
...
template <class T, int Prealloc>
Q_INLINE_TEMPLATE bool QVarLengthArray<T, Prealloc>::contains(ref const(T) t) const
{
    T *b = ptr;
    T *i = ptr + s;
    while (i != b) {
...
template <class T, int Prealloc>
Q_OUTOFLINE_TEMPLATE T QVarLengthArray<T, Prealloc>::value(int i, ref const(T) defaultValue) const
{
    return (uint(i) >= uint(size())) ? defaultValue : at(i);
}

template <class T, int Prealloc>
/+inline+/ void QVarLengthArray<T, Prealloc>::insert(int i, ref const(T) t)
{ Q_ASSERT_X(i >= 0 && i <= s, "QVarLengthArray::insert", "index out of range");
  insert(begin() + i, 1, t); }
template <class T, int Prealloc>
/+inline+/ void QVarLengthArray<T, Prealloc>::insert(int i, int n, ref const(T) t)
{ Q_ASSERT_X(i >= 0 && i <= s, "QVarLengthArray::insert", "index out of range");
  insert(begin() + i, n, t); }
template <class T, int Prealloc>
...
template <class T, int Prealloc>
/+inline+/ void QVarLengthArray<T, Prealloc>::prepend(ref const(T) t)
{ insert(begin(), 1, t); }

template <class T, int Prealloc>
/+inline+/ void QVarLengthArray<T, Prealloc>::replace(int i, ref const(T) t)
{
...
template <class T, int Prealloc>
Q_OUTOFLINE_TEMPLATE typename QVarLengthArray<T, Prealloc>::iterator QVarLengthArray<T, Prealloc>::insert(const_iterator before, size_type n, ref const(T) t)
{
