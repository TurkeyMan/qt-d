...
public import QtCore.qtextstream;
public import QtCore.qstring;
public import QtCore.qvector;
public import QtCore.qset;
public import QtCore.qcontiguouscache;


extern(C++) class export QDebug
{
    friend extern(C++) class QMessageLogger;
...
public:
    ...
    /+inline+/ QDebug(ref const(QDebug) o):stream(o.stream) { ++stream->ref; }
    /+inline+/ QDebug &operator=(ref const(QDebug) other);
    ~QDebug();
    ...
    /+inline+/ QDebug &operator<<(ref const(QString)  t) { maybeQuote(); stream->ts << t; maybeQuote(); return maybeSpace(); }
    /+inline+/ QDebug &operator<<(ref const(QStringRef)  t) { return operator<<(t.toString()); }
    /+inline+/ QDebug &operator<<(QLatin1String t) { maybeQuote(); stream->ts << t; maybeQuote(); return maybeSpace(); }
    /+inline+/ QDebug &operator<<(ref const(QByteArray)  t) { maybeQuote(); stream->ts << t; maybeQuote(); return maybeSpace(); }
...
extern(C++) class QDebugStateSaverPrivate;
extern(C++) class export QDebugStateSaver
{
public:
...
public:
    ...
    template<typename T>
    /+inline+/ QNoDebug &operator<<(ref const(T) ) { return *this; }
}

/+inline+/ QDebug &QDebug::operator=(ref const(QDebug) other)
{
    if (this != &other) {
        QDebug copy(other);
...
template <class T>
/+inline+/ QDebug operator<<(QDebug debug, ref const(QList<T>) list)
{
...
template <typename T>
/+inline+/ QDebug operator<<(QDebug debug, ref const(QVector<T>) vec)
{
    const bool oldSetting = debug.autoInsertSpaces();
...
template <class aKey, extern(C++) class aT>
...
template <typename T>
/+inline+/ QDebug operator<<(QDebug debug, ref const(QSet<T>) set)
{
    const bool oldSetting = debug.autoInsertSpaces();
    debug.nospace() << "QSet";
    debug.setAutoInsertSpaces(oldSetting);
...
template <class T>
/+inline+/ QDebug operator<<(QDebug debug, ref const(QContiguousCache<T>) cache)
{
    const bool oldSetting = debug.autoInsertSpaces();
...
template <class T>
/+inline+/ QDebug operator<<(QDebug debug, ref const(QFlags<T>) flags)
{
