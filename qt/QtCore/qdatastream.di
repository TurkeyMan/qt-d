...
template <typename T> extern(C++) class QLinkedList;
...
extern(C++) class QDataStreamPrivate;
extern(C++) class export QDataStream
{
public:
    enum Version {
        Qt_1_0 = 1,
...
#error Add the datastream version for this Qt version and update Qt_DefaultCompiledVersion
    ...
    QDataStream(ref const(QByteArray) );
    ~QDataStream();
...
template <typename T>
QDataStream& operator<<(QDataStream& s, ref const(QList<T>) l)
{
    s << quint32(l.size());
...
template <typename T>
QDataStream& operator<<(QDataStream& s, ref const(QLinkedList<T>) l)
{
    s << quint32(l.size());
...
template<typename T>
QDataStream& operator<<(QDataStream& s, ref const(QVector<T>) v)
{
    s << quint32(v.size());
...
template <typename T>
QDataStream& operator<<(QDataStream &out, ref const(QSet<T>) set)
{
