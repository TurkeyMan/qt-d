...
extern(C++) class QRegion;

template <typename T>
extern(C++) class QVector
{
...
public:
    ...
    QVector(int size, ref const(T) t);
    /+inline+/ QVector(ref const(QVector<T>) v);
    /+inline+/ ~QVector() { if (!d->ref.deref()) freeData(d); }
    QVector<T> &operator=(ref const(QVector<T>) v);
    ...
    /+inline+/ bool operator!=(ref const(QVector<T>) v) const { return !(*this == v); }
    ...
    /+inline+/ bool isSharedWith(ref const(QVector<T>) other) const { return d == other.d; }
    ...
    void insert(int i, int n, ref const(T) t);
    ...
    QVector<T> &fill(ref const(T) t, int size = -1);
...
#endif
    iterator insert(iterator before, int n, ref const(T) x);
    /+inline+/ iterator insert(iterator before, ref const(T) x) { return insert(before, 1, x); }
    ...
    /+inline+/ ref const(T) first() const { Q_ASSERT(!isEmpty()); return *begin(); }
    /+inline+/ T& last() { Q_ASSERT(!isEmpty()); return *(end()-1); }
    /+inline+/ ref const(T) last() const { Q_ASSERT(!isEmpty()); return *(end()-1); }
    ...
    // comfort
    QVector<T> &operator+=(ref const(QVector<T>) l);
    /+inline+/ QVector<T> operator+(ref const(QVector<T>) l) const
    { QVector n = *this; n += l; return n; }
    /+inline+/ QVector<T> &operator+=(ref const(T) t)
    { append(t); return *this; }
    /+inline+/ QVector<T> &operator<< (ref const(T) t)
    { append(t); return *this; }
    /+inline+/ QVector<T> &operator<<(ref const(QVector<T>) l)
    { *this += l; return *this; }
    ...
    static /+inline+/ QVector<T> fromStdVector(ref const(std::vector<T>) vector)
    { QVector<T> tmp; tmp.reserve(int(vector.size())); std::copy(vector.begin(), vector.end(), std::back_inserter(tmp)); return tmp; }
    /+inline+/ std::vector<T> toStdVector() const
    { return std::vector<T>(d->begin(), d->end()); }
private:
    friend extern(C++) class QRegion; // Optimization for QRegion::rects()
    ...
    extern(C++) class AlignmentDummy { Data header; T array[1]; }
}
...
template <typename T>
/+inline+/ QVector<T>::QVector(ref const(QVector<T>) v)
{
    if (v.d->ref.ref()) {
        d = v.d;
    } else {
...
{ *this = QVector<T>(); }
template <typename T>
/+inline+/ ref const(T) QVector<T>::at(int i) const
{ Q_ASSERT_X(i >= 0 && i < d->size, "QVector<T>::at", "index out of range");
  return d->begin()[i]; }
template <typename T>
/+inline+/ ref const(T) QVector<T>::operator[](int i) const
...
{ Q_ASSERT_X(i >= 0 && i < d->size, "QVector<T>::operator[]", "index out of range");
  return data()[i]; }
template <typename T>
/+inline+/ void QVector<T>::insert(int i, ref const(T) t)
{ Q_ASSERT_X(i >= 0 && i <= d->size, "QVector<T>::insert", "index out of range");
  insert(begin() + i, 1, t); }
template <typename T>
/+inline+/ void QVector<T>::insert(int i, int n, ref const(T) t)
{ Q_ASSERT_X(i >= 0 && i <= d->size, "QVector<T>::insert", "index out of range");
  insert(begin() + i, n, t); }
template <typename T>
...
template <typename T>
/+inline+/ void QVector<T>::prepend(ref const(T) t)
{ insert(begin(), 1, t); }

template <typename T>
/+inline+/ void QVector<T>::replace(int i, ref const(T) t)
{
...
template <typename T>
QVector<T> &QVector<T>::operator=(ref const(QVector<T>) v)
{
    if (v.d != d) {
        QVector<T> tmp(v);
        tmp.swap(*this);
...
template <typename T>
QVector<T>::QVector(int asize, ref const(T) t)
{
    Q_ASSERT_X(asize >= 0, "QVector::QVector", "Size must be greater than or equal to 0.");
    if (asize > 0) {
        d = Data::allocate(asize);
...
template<typename T>
Q_OUTOFLINE_TEMPLATE T QVector<T>::value(int i, ref const(T) defaultValue) const
{
    return uint(i) >= uint(d->size) ? defaultValue : d->begin()[i];
}

template <typename T>
void QVector<T>::append(ref const(T) t)
{
    const T copy(t);
...
template <typename T>
typename QVector<T>::iterator QVector<T>::insert(iterator before, size_type n, ref const(T) t)
{
...
template <typename T>
bool QVector<T>::operator==(ref const(QVector<T>) v) const
{
    if (d == v.d)
        return true;
    if (d->size != v.d->size)
...
template <typename T>
QVector<T> &QVector<T>::fill(ref const(T) from, int asize)
{
    const T copy(from);
...
template <typename T>
QVector<T> &QVector<T>::operator+=(ref const(QVector) l)
{
...
template <typename T>
int QVector<T>::indexOf(ref const(T) t, int from) const
{
    if (from < 0)
...
template <typename T>
int QVector<T>::lastIndexOf(ref const(T) t, int from) const
{
    if (from < 0)
        from += d->size;
    else if (from >= d->size)
...
template <typename T>
bool QVector<T>::contains(ref const(T) t) const
{
    T* b = d->begin();
    T* i = d->end();
    while (i != b)
...
template <typename T>
int QVector<T>::count(ref const(T) t) const
{
    const(T)* b = d->begin();
    const(T)* e = d->end();
...
template <typename T>
QVector<T> QVector<T>::fromList(ref const(QList<T>) list)
{
    return list.toVector();
}

template <typename T>
QList<T> QList<T>::fromVector(ref const(QVector<T>) vector)
{
...
#endif
Q_TEMPLATE_EXTERN template extern(C++) class export QVector<QPointF>;
Q_TEMPLATE_EXTERN template extern(C++) class export QVector<QPoint>;
#endif
