...
\
template <class T> \
extern(C++) class Q##C##Iterator \
{ \
    typedef typename Q##C<T>::const_iterator const_iterator; \
    Q##C<T> c; \
    const_iterator i; \
public: \
    /+inline+/ Q##C##Iterator(ref const(Q##C<T>) container) \
        : c(container), i(c.constBegin()) {} \
    /+inline+/ Q##C##Iterator &operator=(ref const(Q##C<T>) container) \
    { c = container; i = c.constBegin(); return *this; } \
    /+inline+/ void toFront() { i = c.constBegin(); } \
    /+inline+/ void toBack() { i = c.constEnd(); } \
    /+inline+/ bool hasNext() const { return i != c.constEnd(); } \
    /+inline+/ ref const(T) next() { return *i++; } \
    /+inline+/ ref const(T) peekNext() const { return *i; } \
    /+inline+/ bool hasPrevious() const { return i != c.constBegin(); } \
    /+inline+/ ref const(T) previous() { return *--i; } \
    /+inline+/ ref const(T) peekPrevious() const { const_iterator p = i; return *--p; } \
...
\
template <class T> \
extern(C++) class QMutable##C##Iterator \
{ \
...
public: \
    ...
    /+inline+/ void setValue(ref const(T) t) const { if (c->constEnd() != const_iterator(n)) *n = t; } \
    /+inline+/ T &value() { Q_ASSERT(item_exists()); return *n; } \
    /+inline+/ ref const(T) value() const { Q_ASSERT(item_exists()); return *n; } \
    /+inline+/ void insert(ref const(T) t) { n = i = c->insert(i, t); ++i; } \
    /+inline+/ bool findNext(ref const(T) t) \
...
public: \
    /+inline+/ Q##C##Iterator(ref const(Q##C<Key,T>) container) \
        : c(container), i(c.constBegin()), n(c.constEnd()) {} \
    /+inline+/ Q##C##Iterator &operator=(ref const(Q##C<Key,T>) container) \
    ...
    /+inline+/ ref const(T) value() const { Q_ASSERT(item_exists()); return *n; } \
    /+inline+/ ref const(Key) key() const { Q_ASSERT(item_exists()); return n.key(); } \
    /+inline+/ bool findNext(ref const(T) t) \
...
public: \
    ...
    /+inline+/ void setValue(ref const(T) t) { if (const_iterator(n) != c->constEnd()) *n = t; } \
    /+inline+/ T &value() { Q_ASSERT(item_exists()); return *n; } \
    /+inline+/ ref const(T) value() const { Q_ASSERT(item_exists()); return *n; } \
    /+inline+/ ref const(Key) key() const { Q_ASSERT(item_exists()); return n.key(); } \
    /+inline+/ bool findNext(ref const(T) t) \
