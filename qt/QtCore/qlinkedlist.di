...
{
    /+inline+/ QLinkedListNode(ref const(T) arg): t(arg) { }
    QLinkedListNode *n, *p;
    T t;
}

template <class T>
extern(C++) class QLinkedList
{
...
public:
    ...
    /+inline+/ QLinkedList(ref const(QLinkedList<T>) l) : d(l.d) { d->ref.ref(); if (!d->sharable) detach(); }
#if defined(Q_COMPILER_INITIALIZER_LISTS)
...
#endif
    ~QLinkedList();
    QLinkedList<T> &operator=(ref const(QLinkedList<T>) );
    ...
    bool operator==(ref const(QLinkedList<T>) l) const;
    /+inline+/ bool operator!=(ref const(QLinkedList<T>) l) const { return !(*this == l); }
...
#endif
    /+inline+/ bool isSharedWith(ref const(QLinkedList<T>) other) const { return d == other.d; }

    /+inline+/ bool isEmpty() const { return d->size == 0; }

    void clear();

    void append(ref const(T) );
    void prepend(ref const(T) );
    T takeFirst();
    T takeLast();
    ...
    ref const(T) last() const { Q_ASSERT(!isEmpty()); return *(--end()); }
    ...
    static /+inline+/ QLinkedList<T> fromStdList(ref const(std::list<T>) list)
    ...
    // comfort
    QLinkedList<T> &operator+=(ref const(QLinkedList<T>) l);
    QLinkedList<T> operator+(ref const(QLinkedList<T>) l) const;
    /+inline+/ QLinkedList<T> &operator+=(ref const(T) t) { append(t); return *this; }
    /+inline+/ QLinkedList<T> &operator<< (ref const(T) t) { append(t); return *this; }
    /+inline+/ QLinkedList<T> &operator<<(ref const(QLinkedList<T>) l) { *this += l; return *this; }
...
template <typename T>
QLinkedList<T> &QLinkedList<T>::operator=(ref const(QLinkedList<T>) l)
{
    if (d != l.d) {
...
template <typename T>
bool QLinkedList<T>::operator== (ref const(QLinkedList<T>) l) const
{
    if (d->size != l.d->size)
        return false;
    if (e == l.e)
...
template <typename T>
void QLinkedList<T>::append(ref const(T) t)
{
    detach();
    Node *i = new Node(t);
    i->n = e;
...
template <typename T>
void QLinkedList<T>::prepend(ref const(T) t)
{
    detach();
    Node *i = new Node(t);
    i->n = e->n;
...
template <typename T>
int QLinkedList<T>::removeAll(ref const(T) _t)
{
    detach();
    const T t = _t;
    Node *i = e->n;
...
template <typename T>
bool QLinkedList<T>::removeOne(ref const(T) _t)
{
    detach();
...
template <typename T>
bool QLinkedList<T>::contains(ref const(T) t) const
{
    Node *i = e;
    while ((i = i->n) != e)
        if (i->t == t)
...
template <typename T>
int QLinkedList<T>::count(ref const(T) t) const
{
    Node *i = e;
    int c = 0;
    while ((i = i->n) != e)
...
template <typename T>
typename QLinkedList<T>::iterator QLinkedList<T>::insert(iterator before, ref const(T) t)
{
    if (d->ref.isShared())
...
template <typename T>
QLinkedList<T> QLinkedList<T>::operator+(ref const(QLinkedList<T>) l) const
{
