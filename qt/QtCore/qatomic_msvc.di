...
template <int N> struct QAtomicOpsBySize : QGenericAtomicOps<QAtomicOpsBySize<N> >
{
    ...
    static /+inline+/ bool isReferenceCountingWaitFree() nothrow { return true; }
    ...
    static /+inline+/ bool isFetchAndAddWaitFree() nothrow { return true; }
    template <typename T> static T fetchAndAddRelaxed(T &_q_value, typename QAtomicAdditiveType<T>::AdditiveT valueToAdd) nothrow;
...
/+inline+/ bool QAtomicOpsBySize<4>::ref(T &_q_value) nothrow
{
    return QT_INTERLOCKED_FUNCTION(Increment)(atomic(&_q_value)) != 0;
}

template<> template<typename T>
/+inline+/ bool QAtomicOpsBySize<4>::deref(T &_q_value) nothrow
{
    return QT_INTERLOCKED_FUNCTION(Decrement)(atomic(&_q_value)) != 0;
}

template<> template<typename T>
/+inline+/ bool QAtomicOpsBySize<4>::testAndSetRelaxed(T &_q_value, T expectedValue, T newValue) nothrow
{
    return QT_INTERLOCKED_FUNCTION(CompareExchange)(atomic(&_q_value), value(newValue), value(expectedValue)) == value(expectedValue);
}

template<> template <typename T>
/+inline+/ bool QAtomicOpsBySize<4>::testAndSetRelaxed(T &_q_value, T expectedValue, T newValue, T *currentValue) nothrow
{
...
template<> template<typename T>
/+inline+/ T QAtomicOpsBySize<4>::fetchAndStoreRelaxed(T &_q_value, T newValue) nothrow
{
    return QT_INTERLOCKED_FUNCTION(Exchange)(atomic(&_q_value), value(newValue));
}

template<> template<typename T>
/+inline+/ T QAtomicOpsBySize<4>::fetchAndAddRelaxed(T &_q_value, typename QAtomicAdditiveType<T>::AdditiveT valueToAdd) nothrow
{
...
#ifdef Q_ATOMIC_INT16_IS_SUPPORTED
template<> template<typename T>
/+inline+/ bool QAtomicOpsBySize<2>::ref(T &_q_value) nothrow
{
    return QT_INTERLOCKED_FUNCTION(Increment16)(atomic(&_q_value)) != 0;
}

template<> template<typename T>
/+inline+/ bool QAtomicOpsBySize<2>::deref(T &_q_value) nothrow
{
    return QT_INTERLOCKED_FUNCTION(Decrement16)(atomic(&_q_value)) != 0;
}

template<> template<typename T>
/+inline+/ bool QAtomicOpsBySize<2>::testAndSetRelaxed(T &_q_value, T expectedValue, T newValue) nothrow
{
    return QT_INTERLOCKED_FUNCTION(CompareExchange16)(atomic(&_q_value), value(newValue), value(expectedValue)) == value(expectedValue);
}

template<> template <typename T>
/+inline+/ bool QAtomicOpsBySize<2>::testAndSetRelaxed(T &_q_value, T expectedValue, T newValue, T *currentValue) nothrow
{
...
template<> template<typename T>
/+inline+/ T QAtomicOpsBySize<2>::fetchAndStoreRelaxed(T &_q_value, T newValue) nothrow
{
    return QT_INTERLOCKED_FUNCTION(Exchange16)(atomic(&_q_value), value(newValue));
}

template<> template<typename T>
/+inline+/ T QAtomicOpsBySize<2>::fetchAndAddRelaxed(T &_q_value, typename QAtomicAdditiveType<T>::AdditiveT valueToAdd) nothrow
{
...
#ifdef Q_ATOMIC_INT64_IS_SUPPORTED
template<> template<typename T>
/+inline+/ bool QAtomicOpsBySize<8>::ref(T &_q_value) nothrow
{
    return QT_INTERLOCKED_FUNCTION(Increment64)(atomic(&_q_value)) != 0;
}

template<> template<typename T>
/+inline+/ bool QAtomicOpsBySize<8>::deref(T &_q_value) nothrow
{
    return QT_INTERLOCKED_FUNCTION(Decrement64)(atomic(&_q_value)) != 0;
}

template<> template<typename T>
/+inline+/ bool QAtomicOpsBySize<8>::testAndSetRelaxed(T &_q_value, T expectedValue, T newValue) nothrow
{
    return QT_INTERLOCKED_FUNCTION(CompareExchange64)(atomic(&_q_value), value(newValue), value(expectedValue)) == value(expectedValue);
}

template<> template <typename T>
/+inline+/ bool QAtomicOpsBySize<8>::testAndSetRelaxed(T &_q_value, T expectedValue, T newValue, T *currentValue) nothrow
{
...
template<> template<typename T>
/+inline+/ T QAtomicOpsBySize<8>::fetchAndStoreRelaxed(T &_q_value, T newValue) nothrow
{
    return QT_INTERLOCKED_FUNCTION(Exchange64)(atomic(&_q_value), value(newValue));
}

template<> template<typename T>
/+inline+/ T QAtomicOpsBySize<8>::fetchAndAddRelaxed(T &_q_value, typename QAtomicAdditiveType<T>::AdditiveT valueToAdd) nothrow
{
...
struct QAtomicOps<T *> : QGenericAtomicOps<QAtomicOps<T *> >
{
    ...
    static /+inline+/ bool isFetchAndAddNative() nothrow { return true; }
    static /+inline+/ bool isFetchAndAddWaitFree() nothrow { return true; }
    static T *fetchAndAddRelaxed(T *&_q_value, qptrdiff valueToAdd) nothrow;
}

template <typename T>
/+inline+/ bool QAtomicOps<T *>::testAndSetRelaxed(T *&_q_value, T *expectedValue, T *newValue) nothrow
{
    return QT_INTERLOCKED_COMPARE_EXCHANGE_POINTER(&_q_value, newValue, expectedValue) == expectedValue;
}

template <typename T>
/+inline+/ bool QAtomicOps<T *>::testAndSetRelaxed(T *&_q_value, T *expectedValue, T *newValue, T **currentValue) nothrow
{
...
template <typename T>
/+inline+/ T *QAtomicOps<T *>::fetchAndStoreRelaxed(T *&_q_value, T *newValue) nothrow
{
    return reinterpret_cast<T *>(QT_INTERLOCKED_EXCHANGE_POINTER(&_q_value, newValue));
}

template <typename T>
/+inline+/ T *QAtomicOps<T *>::fetchAndAddRelaxed(T *&_q_value, qptrdiff valueToAdd) nothrow
{
