...
public import qt.QtDBus.qdbusargument;
public import qt.QtDBus.qdbuspendingcall;
...
protected:
    ...
    ~QDBusPendingReplyData();
    void assign(ref const(QDBusPendingCall) call);
    void assign(ref const(QDBusMessage) message);
...
public:
    ...
    { }
    /+inline+/ QDBusPendingReply(ref const(QDBusPendingReply) other)
        : QDBusPendingReplyData(other)
    { }
    /+inline+/ /*implicit*/ QDBusPendingReply(ref const(QDBusPendingCall) call) // required by qdbusxml2cpp-generated code
    { *this = call; }
    /+inline+/ /*implicit*/ QDBusPendingReply(ref const(QDBusMessage) message)
    { *this = message; }
    /+inline+/ QDBusPendingReply &operator=(ref const(QDBusPendingReply) other)
    { assign(other); return *this; }
    /+inline+/ QDBusPendingReply &operator=(ref const(QDBusPendingCall) call)
    { assign(call); return *this; }
    /+inline+/ QDBusPendingReply &operator=(ref const(QDBusMessage) message)
...
private:
    ...
    /+inline+/ void assign(ref const(QDBusPendingCall) call)
    {
        QDBusPendingReplyData::assign(call);
        calculateMetaTypes();
    }

    /+inline+/ void assign(ref const(QDBusMessage) message)
    {
