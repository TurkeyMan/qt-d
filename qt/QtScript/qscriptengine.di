...
public import qt.QtScript.qscriptcontext;
public import qt.QtScript.qscriptstring;
public import qt.QtScript.qscriptprogram;
...
template <typename T>
/+inline+/ QScriptValue qScriptValueFromValue(QScriptEngine *, ref const(T) );

template <typename T>
/+inline+/ T qscriptvalue_cast(ref const(QScriptValue) );
...
public:
    ...
    QScriptSyntaxCheckResult(ref const(QScriptSyntaxCheckResult) other);
    ...
    QScriptSyntaxCheckResult &operator=(ref const(QScriptSyntaxCheckResult) other);
...
public:
    ...
    void setGlobalObject(ref const(QScriptValue) object);

    QScriptContext *currentContext() const;
    QScriptContext *pushContext();
    void popContext();

    bool canEvaluate(ref const(QString) program) const;
    static QScriptSyntaxCheckResult checkSyntax(ref const(QString) program);

    QScriptValue evaluate(ref const(QString) program, ref const(QString) fileName = QString(), int lineNumber = 1);

    QScriptValue evaluate(ref const(QScriptProgram) program);

    bool isEvaluating() const;
    void abortEvaluation(ref const(QScriptValue) result = QScriptValue());
    ...
    QScriptValue newFunction(FunctionSignature signature, ref const(QScriptValue) prototype, int length = 0);

    QScriptValue newFunction(FunctionWithArgSignature signature, void *arg);

    QScriptValue newVariant(ref const(QVariant) value);
    QScriptValue newVariant(ref const(QScriptValue) object, ref const(QVariant) value);

#ifndef QT_NO_REGEXP
    QScriptValue newRegExp(ref const(QRegExp) regexp);
#endif

    QScriptValue newObject();
    QScriptValue newObject(QScriptClass *scriptClass, ref const(QScriptValue) data = QScriptValue());
    QScriptValue newArray(uint length = 0);
    QScriptValue newRegExp(ref const(QString) pattern, ref const(QString) flags);
    QScriptValue newDate(qsreal value);
    QScriptValue newDate(ref const(QDateTime) value);
    QScriptValue newActivationObject();

#ifndef QT_NO_QOBJECT
    QScriptValue newQObject(QObject *object, ValueOwnership ownership = QtOwnership,
                            ref const(QObjectWrapOptions) options = 0);
    QScriptValue newQObject(ref const(QScriptValue) scriptObject, QObject *qtObject,
                            ValueOwnership ownership = QtOwnership,
                            ref const(QObjectWrapOptions) options = 0);

    QScriptValue newQMetaObject(const(QMetaObject)* metaObject, ref const(QScriptValue) ctor = QScriptValue());
    ...
    void setDefaultPrototype(int metaTypeId, ref const(QScriptValue) prototype);


    typedef QScriptValue (*MarshalFunction)(QScriptEngine *, const(void)* );
    typedef void (*DemarshalFunction)(ref const(QScriptValue) , void *);



    template <typename T>
    /+inline+/ QScriptValue toScriptValue(ref const(T) value)
    {
        return qScriptValueFromValue(this, value);
    }
    template <typename T>
    /+inline+/ T fromScriptValue(ref const(QScriptValue) value)
    {
        return qscriptvalue_cast<T>(value);
    }

    void installTranslatorFunctions(ref const(QScriptValue) object = QScriptValue());

    QScriptValue importExtension(ref const(QString) extension);
    ...
    QScriptString toStringHandle(ref const(QString) str);
    QScriptValue toObject(ref const(QScriptValue) value);

    QScriptValue objectById(qint64 id) const;

#ifndef QT_NO_QOBJECT
Q_SIGNALS:
    void signalHandlerException(ref const(QScriptValue) exception);
#endif

private:
    QScriptValue create(int type, const(void)* ptr);

    bool convert(ref const(QScriptValue) value, int type, void *ptr);
    static bool convertV2(ref const(QScriptValue) value, int type, void *ptr);

    void registerCustomType(int type, MarshalFunction mf, DemarshalFunction df,
                            ref const(QScriptValue) prototype);

    friend /+inline+/ void qScriptRegisterMetaType_helper(QScriptEngine *,
        int, MarshalFunction, DemarshalFunction, ref const(QScriptValue) );

    friend /+inline+/ QScriptValue qScriptValueFromValue_helper(QScriptEngine *, int, const(void)* );

    friend /+inline+/ bool qscriptvalue_cast_helper(ref const(QScriptValue) , int, void *);
...
template <typename T>
/+inline+/ QScriptValue qScriptValueFromValue(QScriptEngine *engine, ref const(T) t)
{
    return qScriptValueFromValue_helper(engine, qMetaTypeId<T>(), &t);
}

template <>
/+inline+/ QScriptValue qScriptValueFromValue<QVariant>(QScriptEngine *engine, ref const(QVariant) v)
{
    return qScriptValueFromValue_helper(engine, v.userType(), v.data());
}

/+inline+/ bool qscriptvalue_cast_helper(ref const(QScriptValue) value, int type, void *ptr)
{
    return QScriptEngine::convertV2(value, type, ptr);
}

template<typename T>
T qscriptvalue_cast(ref const(QScriptValue) value)
{
    T t;
...
template <>
/+inline+/ QVariant qscriptvalue_cast<QVariant>(ref const(QScriptValue) value)
{
...
#ifdef QT_DEPRECATED
template <typename T>
/+inline+/ QT_DEPRECATED T qScriptValueToValue(ref const(QScriptValue) value)
{
    return qscriptvalue_cast<T>(value);
}
#endif
...
int qScriptRegisterMetaType(
    QScriptEngine *eng,
    QScriptValue (*toScriptValue)(QScriptEngine *, ref const(T) t),
    void (*fromScriptValue)(ref const(QScriptValue) , T &t),
    ref const(QScriptValue) prototype = QScriptValue()
#ifndef qdoc
    , T * /* dummy */ = 0
#endif
)
...
QScriptValue qScriptValueFromSequence(QScriptEngine *eng, ref const(Container) cont)
{
...
template <class Container>
void qScriptValueToSequence(ref const(QScriptValue) value, Container &cont)
{
...
Q_SCRIPT_EXPORT bool qScriptConnect(QObject *sender, const(char)* signal,
                                    ref const(QScriptValue) receiver,
                                    ref const(QScriptValue) function);
