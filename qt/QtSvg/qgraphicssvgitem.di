...
class Q_SVG_EXPORT QGraphicsSvgItem : public QGraphicsObject
{
    ...
    mixin Q_PROPERTY!(QString, "elementId", "READ", "elementId", "WRITE", "setElementId");
    mixin Q_PROPERTY!(QSize, "maximumCacheSize", "READ", "maximumCacheSize", "WRITE", "setMaximumCacheSize");

public:
    QGraphicsSvgItem(QGraphicsItem *parentItem=0);
    QGraphicsSvgItem(ref const(QString) fileName, QGraphicsItem *parentItem=0);

    void setSharedRenderer(QSvgRenderer *renderer);
    QSvgRenderer *renderer() const;

    void setElementId(ref const(QString) id);
    QString elementId() const;

    void setCachingEnabled(bool);
    bool isCachingEnabled() const;

    void setMaximumCacheSize(ref const(QSize) size);
